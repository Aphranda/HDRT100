...
typedef struct {
    ...
    uint64_t count_end;
    uint64_t rollover_start;
    uint64_t rollover_end;
    uint32_t seq_len;
    uint32_t seq_width;
} trig_meas_ctx_t;
...
/* ── 内部: 从硬件读取原始触发计数（无锁原子快照）── */

static uint64_t read_raw_trigger_count(uint64_t *rollover_out)
{
    uint64_t rollover;
    uint32_t seq_idx;
    uint32_t seq_len;

    /* 无锁双读法: 若两次 rollover 不等说明 ISR 恰好在中间
     * 触发, 重试即可消除 ±seq_len 的 race 误差. */
    trigger_vector_t v;
    sync_trigger_get_vector(&v);
    seq_len = v.seq_length;

    do {
        rollover = sync_io_seq_step_get_rollover_count();
        seq_idx  = sync_io_seq_step_get_index();
    } while (rollover != sync_io_seq_step_get_rollover_count());
...
uint32_t trigger_measure_freq_hz(void)
{
    ...
    const uint64_t dcount = s_meas.count_end - s_meas.count_start;

    if (dt_us == 0u || dcount == 0u) {
        return 0u;
    }

    return (uint32_t)(dcount * 1000000ull / dt_us);
}
...
bool trigger_measure_get_report(trigger_measure_report_t *report)
{
    ...
    const uint64_t dt_us = s_meas.t_end_us - s_meas.t_start_us;
    const uint64_t dcount = s_meas.count_end - s_meas.count_start;
    const uint64_t drollover = s_meas.rollover_end - s_meas.rollover_start;
    ...
    report->trigger_count  = (uint32_t)dcount;
    report->rollover_count = (uint32_t)drollover;
    report->elapsed_us     = (uint32_t)dt_us;

    if (dt_us > 0u && dcount > 0u) {
        report->freq_hz  = (uint32_t)(dcount * 1000000ull / dt_us);
        report->period_ns = (uint32_t)(dt_us * 1000ull / dcount);
...
uint32_t trigger_measure_quick_freq_hz(uint32_t gate_ms)
{
    ...
    const uint64_t c0 = read_raw_trigger_count(NULL);

    sleep_ms(gate_ms);

    const uint64_t t1 = time_us_64();
    const uint64_t c1 = read_raw_trigger_count(NULL);

    const uint64_t dt_us = t1 - t0;
    const uint64_t dcount = c1 - c0;
