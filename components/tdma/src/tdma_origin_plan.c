...
static bool config_valid(const tdma_origin_plan_config_t *c, const tdma_origin_plan_t *p)
{
    if (c == NULL || p == NULL || p->runs == NULL || p->literals == NULL ||
        ...
        c->packet_size < TDMA_TRANSPORT_FRAME_HEADER_SIZE ||
        tdma_flight_payload_slots(c->packet_size - TDMA_TRANSPORT_FRAME_HEADER_SIZE) == 0u ||
        c->physical_bytes <= c->outer_header_bytes + c->packet_size ||
        c->physical_bytes > c->packet_size + 64u ||
        c->outer_header_bytes != 4u || c->capture_prefix_bits == 0u ||
        c->capture_prefix_bits > (c->physical_bytes - c->packet_size) * 8u ||
        c->guard_count == 0u || c->guard_count > 65536u ||
        c->abort_poll_count == 0u || c->abort_poll_count > UINT16_MAX ||
        c->local_slot >= tdma_flight_payload_slots(c->packet_size - TDMA_TRANSPORT_FRAME_HEADER_SIZE) ||
        c->active_slot_mask >= (1u << tdma_flight_payload_slots(c->packet_size - TDMA_TRANSPORT_FRAME_HEADER_SIZE)) ||
...
static void emit(builder_t *b)
{
    ...
    case L_PACK_B: {
        ...
        copy(b, a->capture_bank[bank], a->rx_packet, c->packet_size, 1u, 63u, READ | WRITE, loader);
        jump(b, L_IDENTITY);
    }
    break;
    case L_IDENTITY:
    ...
    if (!c->diagnostic_skip_records) {
        move(b, a->rx_packet + c->packet_size - TDMA_FLIGHT_DPLL_OBSERVATION_SIZE,
    ...
    case L_PREPARE_B: {
        ...
        copy(b, a->capture_bank[bank], a->stage + c->outer_header_bytes * 2u + 1u,
             c->packet_size, 1u, 63u, READ | SKIP_WRITE, loader);
        jump(b, L_HEADER);
    }
    break;
    case L_HEADER:
    ...
    copy(b, literal(b, 0u), a->stage + (c->outer_header_bytes + c->packet_size -
        TDMA_FLIGHT_DPLL_OBSERVATION_SIZE) * 2u, TDMA_FLIGHT_DPLL_OBSERVATION_SIZE, 2u, 63u, WRITE, loader);
    copy(b, literal(b, 0u), a->stage + (c->outer_header_bytes + c->packet_size) * 2u,
        c->physical_bytes - c->outer_header_bytes - c->packet_size, 2u, 63u, WRITE, loader);
    ...
    case L_ARM_B: {
        ...
        mark(b, L_ARM_A + bank);
        add(b, STATE(bank_version) + bank * 4u, 1u);
        const uint32_t words[] = {b->cap_ctrl, a->capture_bank[bank], c->packet_size, b->cap_rx};
