...
static bool load(void *context,const sync_io_persona_descriptor_t *d,uint32_t mask)
{
    ...
    if (!d || d->id!=SYNC_IO_PERSONA_ID_SCHEDULED_TRIGGER ||
        ...
        !pio_can_add_program(RUN_PIO,&sync_pulse_stream_out1_program)) return false;
    pio_sm_claim(RUN_PIO,RUN_SM); s_sm_claimed=true;
    dma_channel_claim(RUN_DMA); s_dma_claimed=true;
    s_offset=pio_add_program(RUN_PIO,&sync_pulse_stream_out1_program); s_loaded=true;
    return true;
}
...
static bool arm(void *context,const sync_io_persona_descriptor_t *d,uint32_t mask)
{
    ...
    if (!sync_pulse_stream_out1_program_init(RUN_PIO,RUN_SM,s_offset,RUN_PIN)) return false;
...
static void cleanup(void *context,const sync_io_persona_descriptor_t *d,uint32_t mask)
{
    ...
    if (s_loaded) { pio_remove_program(RUN_PIO,&sync_pulse_stream_out1_program,s_offset); s_loaded=false; }
    ...
    const sync_io_run_output_edge_t edges[SYNC_IO_RUN_OUTPUT_BLOCK_EDGES])
{
    ...
    for (uint32_t i=0;i<SYNC_IO_RUN_OUTPUT_BLOCK_EDGES;++i) {
        ...
        /* Encoder outputs [high countdown, low countdown]. */
    ...
    if (first) {
        ...
        if (__atomic_load_n(&s_cancel,__ATOMIC_ACQUIRE) ||
            ...
            edges[0].rising_tick-before-SYNC_PULSE_STREAM_FIRST_LOW_OVERHEAD>UINT32_MAX ||
