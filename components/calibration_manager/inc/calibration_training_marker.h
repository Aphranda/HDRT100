...
/* The forwarded MARK edge is regenerated by each node's PIO.  A four-node
 * ring can therefore accumulate one-sample (4 ns) edge quantization at each
 * Manchester transition even when every logical bit remains valid.  Keep the
 * raw sample-distance gate explicit and separate from the CRC/Manchester
 * content checks; this budget is for the current four-node acceptance ring. */
#define CALIBRATION_TRAINING_MARKER_MAX_BEST_DISTANCE 1536u
