...
| SYNC-OUT-002 | 为 DPLL/VDC 提供统一 owner 的有限持续 PIO 输出 | IN PROGRESS | STOP 预留、完整客户端交接、START 准入、源缓冲退休与不可改写前缀及有界退休已实现；uniform 固定高宽单字 FIFO 已通过 host、资源和同源码 quick P3，实际有限 RUN 仍四板 STARVED。下一步核对模型后缀重建、dispatcher 服务间隔与 DMA 退休期限的关联；联动 `VDC-OUTPUT-001`，有限实测不代替 WCET 或物理精度。 |
