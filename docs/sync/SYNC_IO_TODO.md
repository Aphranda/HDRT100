...
| SYNC-M2 | PIO0 persona 资源与生命周期收敛 | PENDING | descriptor、兼容矩阵、原子 claim/release、失败回滚和无泄漏负测通过。 |
...
| SYNC-RES-001 | 建立 PIO0 persona descriptor 与兼容矩阵 | PENDING | 每个 persona 声明 SM、instruction、GPIO read/write、FIFO、DMA/DREQ、IRQ、workspace 和 safe state；静态冲突负测通过。 |
| SYNC-RES-002 | 建立 PIO0 persona lifecycle manager | PENDING | validate/claim/load/arm/stop/release 原子化；失败保持 STOPPED 或恢复旧 persona，无部分 claim。 |
| SYNC-RES-003 | 将 `WAVE_OUTPUT` / `SCHEDULED_TRIGGER` 从 TDMA TX PIO 迁入 PIO0 owner | PENDING | 不直接复制旧 SM 编号；输出 HIL、停止安全态和 TDMA 同时运行验证通过；删除 suspend/resume handoff。 |
