...
| SYNC-LA-004 | 实现 `TRIGGERED_CAPTURE` 前后窗口 | IN PROGRESS | level/edge/pattern trigger、bounded pre/post window、timeout/end reason 和重复 ARM 测试通过。 |
...
| SYNC-OUT-002 | 为 DPLL/VDC 提供统一 owner 的有限持续 PIO 输出 | IN PROGRESS | STOP 预留、完整客户端交接、START 准入、源缓冲退休与不可改写前缀及有界退休已实现；进度 `SYNC-PROGRESS-20260918-004` 长时间轴、分批后缀与 SCPI/Flash 配置已有同源码 quick P3、保存恢复及四板十边沿块实际采用验证，caller wall 未超候选预算，但仍四板 STARVED。下一步核对模型更新后的后缀重建与 DMA 退休后的真实补给期限；联动 `VDC-OUTPUT-001`，有限实测不代替 WCET 或物理精度。 |
