...
每个业务位置完成整个 SP8T 序列，各状态采一次。START 建立运行身份和计数锚点、准备首项，
...
选择未来目标；过期目标拒绝，不立即补发。COUNTER、DUT、VNA 并行准备同一预约，满足
目标参与者准备与运输证明后提交。VNA 本地动作须等 DUT 预约编码的稳定条件成立，
...
中间窗口每位置执行完整SP8T采样计划，READY后预约下一步。末个业务位置的末项READY
