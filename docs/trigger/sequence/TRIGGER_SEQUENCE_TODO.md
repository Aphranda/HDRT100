...
| NSEQ-RISK-06 | 历史`CONF:SEQ:LINK OFF`无回复保留；进度066已返回本次拒绝阶段、TDMA/gateway/回退结果，主机可区分锁竞争及代次等原因 | OPEN：已补明确回复与确定性回归，尚未在原间歇OFF拒绝发生时取得新诊断；保留反复配置和外部静默闭环要求，不将诊断改善等同根因消除 |
...
NSEQ-RISK-06仍OPEN：下一代码切片继续定位原间歇OFF拒绝，保留本次诊断，不以成功重跑消除该风险；
部署/VDC迁移及响应提速仍须逐片通过外部静默验证。
