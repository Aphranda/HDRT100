...
| NSEQ-RISK-06 | 进度065的固定验收r3中，STOP及环路停止读回后`CONF:SEQ:LINK OFF`产生执行错误而无回复；尚未区分配置准入、LINK writer guard及内部配置拒绝 | OPEN：下一切片补可归因的配置拒绝结果及并发回归，反复配置后完成静默转台闭环；本次单板凭证不代表间歇配置拒绝已经修复 |
