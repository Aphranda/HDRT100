...
当前实现包含独立SP8T、DUT/VNA组合与COUNTER/DUT/VNA三槽位位置模式。
按用户最新确认，新增第四种共享VDC预约触发模式，设计与实施门禁见NSEQ-120至125；
第四模式尚未实现，不把现有普通LINK消息流程标为预约触发。
...
| NSEQ-RISK-04 | 进度066复现质量快照不可用；进度067窄质量读取修复无关锁依赖，相关竞争负测及当前单板门禁通过；进度068补齐同build外部静默闭环 | 已关闭本次无关锁依赖缺陷：修复、质量字段映射/竞争负测、连续配置及外部短轮次证据闭合；真正质量失败和有界读取耗尽仍拒绝，不保证任意并发重绑或全部间歇配置问题消除 |
| NSEQ-RISK-06 | 进度069修复稳定OFF后台写者；快速档位预检在启用但运行已结束时再捕获WRITER_BUSY，进度071补齐清理后IDLE静默 | 已关闭本次后台空转争用：配置换代、STOP/START和最终计数变化唤醒的软件负测、新固件单板凭证、外部闭环及后续配置通过；首次必要清理和真实冲突仍可拒绝，不保证全部配置永不BUSY |

本轮先执行配置诊断切片：LINK 返回本次拒绝阶段、TDMA stopped-config 原因、gateway 及回退结果；
ROLE ACT 区分质量快照不可用与真实门禁失败，并保留本次尝试的分项证据，不引用旧 registry 错误归因。
配置修改不自动重放，既有冻结、停止确认和代次检查继续有效。软件回归后构建带 USBTMC 的固件，
经真实 OTA 反复配置验证，再用现有外部转台接线完成静默短轮次及历史核验；按用户授权接受单板功能证据。
本切片不将诊断可见性提升等同于间歇失败根因已消除，后续风险状态以进度记录和原始证据为准。
进度066已捕获质量快照不可用；继续在同一配置修复切片中引入窄质量读取，
保留全局累计计数和已配置scheduler的traffic-class错误映射，不改为忽略错误或清零累计数。
仅复核相关intent/result发布身份，未取得一致数据仍BUSY；测试须比较窄读取与旧完整读取的质量字段。
同时修复验收工具对标准无回复`*CLS`的处理，发送后独立查询错误队列，不伪造设备回复。
进度067已完成上述修复、真实OTA和当前源码单板凭证，代码已按既有授权带risk提交推送。
进度068在用户开启信号源后完成同build独立IN1及第三模式外部静默短轮次验证，闭合NSEQ-RISK-04。
进度069已在停止态OFF压力工具中捕获WRITER_BUSY并修复无效后台writer竞争；配置与服务的
已发布启用态判定保持同一临界区，保留启用态STOP清理、真实锁冲突拒绝及首次失败停止。
负向回归、新build/OTA、同参数压力、固定单板凭证及外部静默短轮次通过，仅闭合稳定OFF子路径。
随后启用但已结束的IDLE状态复现同类争用，NSEQ-RISK-06重新打开；进度071已补齐该路径
修复、软件负测、当前固件单板凭证及外部静默闭环，再次关闭已确认的后台空转争用。
之后结合NSEQ-120至125推进物理部署/业务槽位映射、共享VDC及特等席预约；响应提速仍未达标，
每片继续外部静默验证。旧无分项拒绝证据保留，不将本次修复等同所有配置路径永不忙。
...
用户于2026-09-19进一步确认：本批从多板TDMA复用到单板序列的共享实现，可按真实单板
闭环验收。`sequence_single_board_gate.py`逐文件纳入本批实时入口、TDMA运行绑定、TIMER1
及诊断/GUI依赖；不按目录放行，不取消源码指纹、包摘要、真实OTA及固定profile校验。
未来共享文件内的其他用途仍须按实际diff审查，不能仅凭文件在白名单内就声称多板已验证。
...
| NSEQ-097 | 分环节计时与特等席迁移基线 | IN PROGRESS | 进度058/059已迁移序列TIMER1计时并完成新build的360位置统计；旧tick估算保留在进度056。仍缺PIO/OUT/READY硬件边沿时间戳和物理波形，SCPI毫秒取整不能替代微秒级边沿测量 |
...
| NSEQ-110 | 序列运行计时迁移TIMER1 | IN PROGRESS | LINK超时、历史/消息计时及RefMem发布间隔已采用TIMER1，Core1硬件周期释放与real-flight对齐。软件回归、USBTMC构建/OTA及外部1kHz完整360位置通过，见进度058/059；当前提交凭证仍待闭合，其他模式和全局Core0 RTOS时钟不据此扩展验收 |

### 统一多板架构的长期迁移任务

任务目标：单板作为多板 TDMA/VDC 的一种部署，完成序列所需统一运输、共享时间、实时流水线
及验收。以下为实施计划，非冻结契约；保留 NSEQ-101 至 NSEQ-110 的未闭合项及原始失败，
不以新任务编号取消旧风险。每个代码切片先软件回归、构建、OTA和当前固件短帧闭环，并且
必须用外部转台脉冲完成第三模式真实RJ45/READY闭环，再进入下一切片。用户指定每切片运行
30位置以快速迭代（验收配置快照，非产品常量）；至少检查相邻位置、
每位置完整计划、累计脉冲/阈值历史、末项自然结束及停止清理；软件注入不替代外部脉冲。
阶段性闭环不替代NSEQ-116完整扫描，失败原件保留，未修复不得继续下一迁移切片。

| ID | 任务 | 状态 | 完成或退出门禁 |
|---|---|---|---|
| NSEQ-111 | 多板模型差异与迁移基线 | DONE | 已核对 real-flight 参考发布、每板 DCO、身份和邮箱占用；本节及架构草案记录设计，进度060记录审计证据；DONE仅指规划完成 |
| NSEQ-112 | 统一 TDMA 部署、运行绑定与路由 | IN PROGRESS | 首切片冻结已应用环路身份，STEP/FIRE/COUNTER_REARM实际提交与TDMA STOP互斥；软件回归、USBTMC构建/OTA和外部1kHz三十位置闭环通过，见进度061。START生命周期尚不具备同等提交保护；后续业务槽位映射物理板，统一来源/目标校验、回程、去重及背压；不得把单板逻辑双节点配置当作多板验收 |
| NSEQ-113 | 共享 VDC 坐标、模型及唯一参考绑定 | PENDING | TIMER1原点一致，稳定committed DCO与发布版本可读；每环一个参考、同板角色共享；错误来源/代次/模型及失效负测通过，禁止复制远端local anchor |
| NSEQ-114 | VDC和序列运输共存及有界流水线 | PENDING | 固定空间/运输配额与唯一装配者，避免priority接管邮箱饿死序列；分离短步/生命周期，保留PIO0热加载、STOP优先、原phase与guard；承接NSEQ-101/102/103 |
| NSEQ-115 | 硬件分段计时与响应提速 | IN PROGRESS | PIO0/TIMER1基础拍、HIST:TIM?及分段工具已完成，短/长脉宽对照见进度062；Trigger诊断向量与静默三十位置通过，见进度064，主机运行期零命令且完整记录零覆盖。缩短脉宽未消除毫秒级调度/运输等待，物理READY响应目标仍未通过；后续优化有界流水线并补物理边沿证据。纳秒脉宽配置、主动诊断压力下BUSY仍保留边界 |
| NSEQ-116 | 三模式单板完整与压力验收 | PENDING | 当前固件真实RJ45及IO回环；首末项/有限连续/暂停停止/忙时位置故障/恢复/热加载；第三模式完整扫描及按实际源参数逐档提速，承接NSEQ-104 |
| NSEQ-117 | SCPI、GUI和提交交付 | IN PROGRESS | 进度065完成当前源码的三模式固定单板凭证及额外静默三十位置，代码检查点ae156377带risk提交；SCPI/GUI参数及JSON保存、计时向量和工具已纳入。配置间歇拒绝仍由NSEQ-RISK-04/06跟进，最终交付待长期迁移完成 |

当前诊断迁移切片按用户追加要求使用HAOFV本地Trigger诊断向量隔离：Core1单写，Core0
有界只读；不读取运行态owner、不取得实时锁，不占用TDMA/VDC载荷。保留完整短轮次记录，
先核对向量容量，再启动；运行期间主机不查询，结束后读回记录与身份、覆盖计数。
`--diagnostic-stress`明确属于主动干扰压力探针，默认关闭，不能用来证明正常性能。

NSEQ-114/115的下一性能切片须区分Core1服务周期、TDMA档位周期及RefMem发布节流，不能将三者
统称为TDMA周期。当前`app_realtime_tdma_phase`先运行TDMA/RefMem，再运行sequence owner/LINK，
新LINK消息等待后续flight服务；`distributed_refmem_tdma_flight_sync_publish`又将发布间隔下限
绑定到向上取整的`feedback_timeout_ns`。按统一运输配额迁移事件交付和同阶段有界流水线，
保留真实RJ45回程、背压和guard，不通过取消WCET门禁或单独缩短总周期取得表面提速；审计见进度070。
静默采集也保留硬件计数读取和RAM发布成本，不宣称绝对零开销；异常与容量不足必须失败。
该切片已完成新构建、OTA、外部三十位置及完整历史验证，见进度064；后续迁移默认使用
`--quiet-capture`取得静默短轮次基线，容量不足则拒绝，不以流式采集冒充静默。

验收参数快照（用户目标，非产品常量）：READY响应低于1ms；OUT脉宽10us和1000us对照；
IN1外部1kHz、N=1000、360位置、每位置完整SP8T计划，OUT4到IN2模拟READY及RJ45物理回环。
执行前核对真实设备与源参数，后续频率改变经SCPI调整；不以增大N掩盖整轮执行超时。
既有360位置功能证据属于进度059对应旧build，不能替代后续迁移固件验收。单板不声明
多板物理同步、独立边沿计数或RF通过；代码提交仍须匹配实际staged范围的真实门禁凭证。
...
进度065已关闭本批源码的提交凭证缺口；固定软件计数profile使用`POSITION_COUNTER_INPUT`
选择未接源输入，避免与持续外部源混计，READY仍为OUT4到IN2真实回接。独立的静默外部
三十位置继续使用IN1，不以注入profile代替真实转台闭环。下一切片先处理配置拒绝风险，
不能因固定profile一次通过就关闭间歇失败。
...
语义草案见架构NSEQ-RSV-01至05。用户确认的主线是：连续转台脉冲与已知计划用于提前
规划位置T2；每位置跑完整组合采样序列；每次VNA READY后授权并预约下一步。按用户新增要求，
第四模式使用COUNTER、SP8T、SP2T、VNA四个业务槽位，共享既有VDC时间服务并行准备，
按各自deadline执行；两个开关电平均稳定后才允许VNA采样。单板配置规划为SP8T用OUT1/OUT2
选择四个状态、SP2T用OUT3切H/V极化、VNA用OUT4，转台IN1与READY IN2分离。
每个SP8T状态按H、V分别采样，再进入下一SP8T状态，本轮每位置八次采样；H/V电平映射
可配置/读回，不能默认硬件极性或把四个SP8T状态当八路再翻倍。前三模式配置语义保留。READY捕获
及快速交接归SYNC域，走与VDC同圈的特等席；Trigger保留业务推进裁决。前三模式保留。
以下为实施计划，不是冻结接口，不预先添加返回成功的SCPI或可启动的GUI空壳。

| ID | 任务 | 状态 | 完成或退出门禁 |
|---|---|---|---|
| NSEQ-120 | 第四模式需求、T2模型与owner审计 | DONE | 来源7a3e0954同步和T2接口审计完成，见NSEQ-RSV-06、进度074；第四槽SP2T切H/V，SP8T二位四状态各采H/V，OUT1/2、OUT3、OUT4分别归两开关及VNA；模式独立PIO热加载，资源/角色实现仍由后续任务验证，不表示固件完成 |
| NSEQ-121 | 共享VDC稳定视图与位置预约规划 | PENDING | 承接NSEQ-113；参考real-flight生产调用链的TIMER1同原点/committed发布及维护；先闭合无输入aging、失参考及ready(false)完整质量发布；区分token/valid_from_raw/publication_revision，逆映射local ns再转tick；有限有界前瞻、溢出/量化、校准/模型/schedule换代及预测偏差负测；已承诺前缀冻结，诊断未锁定输出不作为正式ARM准入 |
| NSEQ-122 | SYNC READY特等席及预约运输 | PENDING | 承接NSEQ-112/114；VNA READY、运输offer READY、预约READY-mask/fence分离；typed VDC完整body不可直接复用，新增固定布局与唯一装配者、来源/步骤/运行去重及容量预算共存；真实RJ45交接，实测到达相位/最坏圈数，无普通发布节流依赖、不吞缺口 |
| NSEQ-123 | 并行预约FB及PIO0本地执行 | PENDING | 先核算组合预约persona的SM/指令/DMA/FIFO及VDC采集共存，当前三槽位已占满PIO0的SM，不能叠加SCHEDULED_TRIGGER；停态装载且运行期持续计数/捕获；READY只授权一次，并行PREPARE/fence/ARM、两开关均满足各自稳定期限后VNA触发；硬件deadline、STOP取消/PAUSE/CONT/热加载/迟到及VDC失效负测；不把软件观察时间当边沿 |
| NSEQ-124 | 第四模式SCPI、GUI与配置保存 | PENDING | 模式独立分页；四槽角色/部署、SP8T二位编码及SP2T档位/极性/安全态/各自settle、组合计划、输入/频率或周期、lead/超时、总观察位置及前后VDC窗口配置/真实读回/JSON恢复；派生业务位置数与绝对计数偏移可见，无业务窗口拒绝；能力未就绪明确拒绝，前三模式回归 |
| NSEQ-125 | 预约模式静默闭环与时间拆解 | PENDING | 每片保留第三模式外部短轮次回归；预约可执行后增加第四模式前置VDC/业务/后置VDC分段验收；首业务触发定位及末段稳态、READY与圈序号、装载、编码稳定、VNA输出、误差/超期逐项证据，最终完整扫描及前三模式回归 |

资源核算按用户确认的各模式独立PIO热加载执行：仅核算当前模式的同时驻留需求，
第四模式独立设计组合预约persona；第三模式占满SM不等于第四模式无法装载。

进入顺序：NSEQ-RISK-06当前IDLE争用切片已按进度071闭合，NSEQ-120上游接口审查及
新增SP2T四槽组合顺序已完成，从NSEQ-121至125逐片推进。
SP2T复用既有LINK_SWITCHER能力的可行性、实例区分和单一输出owner须在编码前核对，
READY/fence绑定四槽配置及实际输出参与者，不能把两开关误判为重复角色或共享输出掩码。
不跳过NSEQ-112至117仍适用的运输/时钟/验收门禁。
每次实现切片都要软件回归、USBTMC构建/OTA、当前源码单板凭证和外部静默30位置
（用户验收配置快照，非产品常量），测试期间主机不查询TDMA/PHY。
第四模式未具备端到端能力前，只能报告实际实现层及第三模式回归；不得宣称预约验收通过。
每位置时间分解保留预测、READY、运输、准备/装载、硬件执行与采样等待的不同坐标和证据类型。
性能目标承接NSEQ-115；共享VDC或同圈运输本身不代表已消除Core1慢周期，也不证明相位精度。

第四模式验收配置快照（用户要求，非产品常量）：总观察位置30、前置5、后置5，
中间20个位置每位置完整采样；前后只生产/验证VDC，不计业务采样。后续允许缩为前2后2，
若总数仍为30则派生26个业务位置。首段负责实际输入锚点与首触发定位，末段负责连续
稳态证据；缩窗不降低质量门禁，不用GUI START时刻作为首个输入脉冲时间。
用户报告real-flight四板已达到±25ns；单板4至8ns为本模式预期目标，须核对上游证据身份
并独立测量，不等于当前分支能力。基础拍、映射误差、槽位动作偏差和物理IO误差分别验收。
