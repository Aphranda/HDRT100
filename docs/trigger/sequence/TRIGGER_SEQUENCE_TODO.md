...
规划位置T2；每位置跑完整SP8T序列；每次VNA READY后授权并预约下一步。三个槽位共享
VDC并行准备、按各自deadline执行；DUT编码至VNA采样的稳定时间仍须满足。READY捕获
及快速交接归SYNC域，走与VDC同圈的特等席；Trigger保留业务推进裁决。前三模式保留。
以下为实施计划，不是冻结接口，不预先添加返回成功的SCPI或可启动的GUI空壳。

| ID | 任务 | 状态 | 完成或退出门禁 |
|---|---|---|---|
| NSEQ-120 | 第四模式需求、T2模型与owner审计 | IN PROGRESS | 对齐T2 canonical、连续脉冲预测/实测位置、READY后授权、并行动作依赖及单板/多板边界；前置/业务/后置窗口可配置；独立复核，不以文档表示固件完成 |
| NSEQ-121 | 共享VDC稳定视图与位置预约规划 | PENDING | 承接NSEQ-113；参考real-flight的VDC生产/维护/committed发布，唯一参考、本板正反映射、硬件锚点/周期、有限及有界前瞻、整数溢出/量化、模型换代与预测偏差负测；无效质量不ARM |
| NSEQ-122 | SYNC READY特等席及预约运输 | PENDING | 承接NSEQ-112/114；固定字段与唯一装配者、来源/步骤/运行去重、与VDC容量共存、真实RJ45交接；实测到达相位/最坏圈数，无普通发布节流依赖、不吞缺口 |
| NSEQ-123 | 并行预约FB及PIO0本地执行 | PENDING | READY只授权一次；并行PREPARE/fence/ARM、DUT稳定后VNA触发、持续转台计数共存；硬件deadline执行、STOP取消/PAUSE/CONT/热加载/迟到及VDC失效负测；不把软件观察时间当边沿 |
| NSEQ-124 | 第四模式SCPI、GUI与配置保存 | PENDING | 模式独立分页；输入/计划/频率或周期、lead/settle/超时、总观察位置及前后VDC窗口配置/真实读回/JSON恢复；派生业务位置数与绝对计数偏移可见，无业务窗口拒绝；能力未就绪明确拒绝，前三模式回归 |
...
进入顺序：NSEQ-RISK-06当前IDLE争用切片已按进度071闭合，接着完成NSEQ-120审查，再按
NSEQ-121至125逐片推进；不跳过NSEQ-112至117仍适用的运输/时钟/验收门禁。
