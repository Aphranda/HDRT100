...
| `SYSTem:REFMEM:CLAIM? [slot_id]` | 查询 SlotClaimMap 首版派生视图；省略时查询 A0。返回 `version,claim_epoch,slot_count,candidate_count,assigned_count,conflict_count,overflow_count,disabled_count,map_crc32,slot_id,board_id,board_uuid_crc32,capability_mask,io_constraint_mask,ip_core_mask,loaded_instance_mask,claim_count,claim_state,reason,claim_policy,claim_priority,slot_claim_epoch,last_claim_seq,claim_crc32`。 |
| `SYSTem:REFMEM:LOAD:SD [path]` | 只在 RefMem load 状态机 `mode=IDLE` 且实时触发空闲时允许；先扫描 SD System Pack manifest，再读取并校验 RefMem table image，默认路径 `/refmem/app_model.rmtp`。只写 staging load snapshot，不直接覆盖 active 表。返回 `STAGED/REJECTED` 加 load snapshot。 |
...
| `SYSTem:REFMEM:LOAD:BOARD <board_id>,<board_uuid_crc32>,<capability_mask>,<io_constraint_mask>,<ip_core_mask>,<default_persona_mask>,<hw_profile_crc32>,<active_default_slot>,<online_required>` | 通过 SCPI 提交一条 BoardCapability 候选到 RefMem staging，用于调试、System Pack 前置验证和板卡能力实例化；必须包含 `REFMEM+VDC` baseline，且只进入 staging，不直接修改 active BoardCapabilityTable。返回 `STAGED/REJECTED` 加 board load snapshot。 |
...
| `SYSTem:PROTection:STATus?` | 查询 `RuntimeProtectionTable` 快照：`version,table_seq,ram_resident_required,flash_lockout_supported,flash_lockout_online,flash_lockout_requested,flash_lockout_acknowledged,park_state,entry_table_owner,flags,guard_owner,guard_crc,guard_stale,guard_flags`。 |
