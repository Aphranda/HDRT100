...
| `SYSTem:REFMEM:LOAD:SD [path]` | 只在 RefMem load 状态机 `mode=IDLE` 且实时触发空闲时允许；扫描 SD System Pack manifest，将结果写入 RefMem staging load snapshot，不直接覆盖 active 表。返回 `STAGED/REJECTED` 加 load snapshot。 |
...
| `SYSTem:REFMEM:TABle? [table_id]` | 查询 RefMem TableRegistry 首版 snapshot 和指定表 entry。返回 `version,table_count,active_table_mask,staging_table_mask,registry_crc32,last_error,table_id,owner,layout_version,active_crc32,staging_crc32,validation_state,validator_id,last_result,evidence_index,flags`。 |
