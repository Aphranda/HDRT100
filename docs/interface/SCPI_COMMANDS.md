...
| `SYSTem:REFMEM:NODE? [node_id]` | 查询 RefMem 内部主域的 NodeSlot P0 快照；省略时读取本节点，当前预留 A0-A7 八个通用节点，真实板卡、模型网分、模拟转台、网关和测试代理作为 role/persona/instance 载入节点。返回 `node_id,state,heartbeat,slot_version,last_update_ms,stale_count,fault_code,flags,node_type`。 |
