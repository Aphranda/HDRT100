...
| `SYSTem:WATCHdog:STATus?` | 查询看门狗运行和上次复位证据：`enabled,last_reset_watchdog,last_reset_timeout,timeout_ms,reset_reason,evidence_magic,evidence_expected_mask,evidence_seen_mask,evidence_stale_mask,evidence_core0_loop_count,evidence_core1_loop_count,gate_required_mask,last_seen_mask,last_stale_mask,supervisor_count`。`evidence_stale_mask` 按任务位标识复位前未推进的执行体，任务位顺序见 `diagnostics_watchdog_task_t`；维护/低优先级任务只记录证据，硬复位门槛是 system supervisor 和 core1。 |
| `SYSTem:WATCHdog:LOG?` | 查询可读的上次复位日志：`reset_type,cause,stalled_task,raw_reason,evidence_valid,expected_mask,seen_mask,stale_mask,core0_loop_count,core1_loop_count`。`cause` 可区分 `CORE0_SUPERVISOR_STALL`、`CORE1_STALL`、`TASK_STALL` 和证据缺失。 |
| `SYSTem:TDMA:FLIGHT:PROCess?` | 查询 EtherCAT-style 固定偏移处理证据：`version,configured,active,local_slot,map_crc32,map_generation,payload_size,local_segment_count,map_apply_count,input_bytes,output_bytes,tx_stale_reuse_count,map_reject_count,length_reject_count,tx_unavailable_count`。只读；运行态不提供 map 写入口。 |
