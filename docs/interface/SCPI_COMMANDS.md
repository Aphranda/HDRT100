...
| `SYSTem:LED:STATus?` | 查询三灯维护策略快照：`policy_name,policy,system_pattern_name,system_pattern,system_level,arm_pattern_name,arm_pattern,arm_level,fault_pattern_name,fault_pattern,fault_level,fault_latched,trigger_state,ota_state,config_ready,sd_ready,core1_stale,health_flags,event_sequence,trigger_pulse_count,fault_transition_count,pattern_transition_count`。这是只读状态，不提供任意写灯入口。 |
| `SYSTem:WATCHdog:STATus?` | 查询看门狗运行和上次复位证据：`enabled,last_reset_watchdog,last_reset_timeout,timeout_ms,reset_reason,evidence_magic,evidence_expected_mask,evidence_seen_mask,evidence_stale_mask,evidence_core0_loop_count,evidence_core1_loop_count,evidence_core0_progress,evidence_core1_progress,gate_required_mask,last_seen_mask,last_stale_mask,supervisor_count`。`evidence_stale_mask` 按任务位标识复位前未推进的执行体，任务位顺序见 `diagnostics_watchdog_task_t`；`evidence_core0_progress/evidence_core1_progress` 是复位前最后写入硬件 scratch 的完整进度标记。progress 编码固定为 `(execution_id << 8) | phase`：core0 的 `execution_id=1..10` 依次对应 system、USB device、SCPI、REFMEM sync、loop engine、calibration、config gate、OTA、storage、UI，core1 使用 `execution_id=1` 表示实时流水线；低 8 位 phase 的代码锚点为 `app_tasks.c`、`app.c` 和 `status_ui.c`。维护/低优先级任务只记录证据，硬复位门槛是 system supervisor 和 core1。 |
| `SYSTem:WATCHdog:LOG?` | 查询可读的上次复位日志：`reset_type,cause,stalled_task,raw_reason,evidence_valid,expected_mask,seen_mask,stale_mask,core0_loop_count,core1_loop_count,core0_progress,core1_progress`。`cause` 可区分 `CORE0_SUPERVISOR_STALL`、`CORE1_STALL`、`TASK_STALL` 和证据缺失。 |
| `SYSTem:TDMA:FLIGHT:PROCess?` | 查询 EtherCAT-style 固定偏移处理证据：`version,configured,active,local_slot,map_crc32,map_generation,payload_size,local_segment_count,map_apply_count,input_bytes,output_bytes,tx_stale_reuse_count,map_reject_count,length_reject_count,tx_unavailable_count,rx_bitmap_scan_count,rx_bitmap_hit_count,rx_bitmap_duplicate_count`。只读；运行态不提供 map 写入口。位图 hit 仅在 RX descriptor 成功入 FIFO 后提交，FIFO 满导致的未交付候选允许同 seq16 重试。 |
| `SYSTem:REFMEM:SYNC:FLIGHT?` | 查询 8-slot compact mailbox 的 core0 消费快照：`enabled,local_slot,node_count,active_mask,reference_slot,remote_slot,payload_size,mailbox_size,publish_interval_ms,next_seq32,tx_publish_count,tx_reject_count,rx_acquire_count,rx_empty_count,rx_accept_count,rx_reject_count,rx_duplicate_skip_count,rx_bad_mailbox_count,last_rx_result,last_frame_type,last_source_slot,last_seq32,last_value_u32,last_error`。`remote_slot` 仅为兼容诊断字段，调度与解析必须使用 active/segment mask。 |
...
| `SYSTem:SYNC:VDC:OBServer:TDMA:SELFtest [role],[output_index],[observed_mask],[initial_sample_mask],[sample_period_ns],[pulse_period_ns],[pulse_high_ns],[pulse_count],[frame_crc32],[start_delay_ns],[phase_only],[phase_max_span_ns],[phase_min_stable_rounds]` | 维护态 VDC/TDMA bring-up。`phase_only=0` 保留 TDMA 诊断路径；`phase_only=1` 时 NO1-NO4 用 `PIO0/SM1` 从各自当前 DCO 相位持续驱动 SMA OUT1，NO5 用 `PIO0/SM0` 捕获四路，不向 TDMA/DPLL 提交观测样本。首轮相位差只记录，连续稳定轮才参与门禁；`role=0,phase_only=1` 停止并释放该观测角色。 |
| `SYSTem:SYNC:VDC:OBServer:TDMA:SELFtest?` | 查询 self-test 最近一次配置和计划摘要，并返回 phase 阈值及已调度脉冲数。 |
...
| `SYSTem:SYNC:VDC:OBServer:PHASe?` | 查询 NO5 外部相位观测：轮数、完整/缺失/歧义计数、初始/最终/峰值/最小跨度、四路初始和最终偏差、连续稳定轮、稳定段抖动、收敛状态及配置阈值。相位按周期圆周距离计算，允许跨相邻周期边界分组。 |
| `SYSTem:SYNC:VDC:OBServer:WAVEform:ARM` | 在 NO5 启动 PIO0 原始波形分段记录。Core0 将 edge-qualified raw word 及完整硬件时间元数据追加到双 SRAM 缓冲，StorageAO 异步写 SD；不查询或修改 Core1 环路状态。返回 `OK,session_id,segment_record_capacity`。 |
| `SYSTem:SYNC:VDC:OBServer:WAVEform:STOP` | 停止接受新的原始记录并请求冲刷剩余分段。返回 `OK,record_count,dropped_count`；完成状态必须继续由 `STATus?` 查询。 |
| `SYSTem:SYNC:VDC:OBServer:WAVEform:STATus?` | 只读查询 `armed,stopping,complete,session_id,record_count,dropped_count,segment_count,pending_record_count,first_sample_seq,last_sample_seq,start_ms,end_ms,last_error,last_job_id,last_path`。该状态仅用于进度和完整性检查。 |
| `SYSTem:SYNC:VDC:OBServer:WAVEform:SAVE` | 在 STOP 冲刷完成后返回 `OK,segment_count,path_prefix`。主机按连续段号下载 SD 二进制文件，校验每段 CRC 后从 raw word 重建四路边沿和相位曲线。 |
...
| `SYSTem:REFMEM:STATus?` | 查询 RefMem 内部主域的本地 DistributedVectorTable P0 快照：`table_size,layout_version,table_seq,local_node_id,node_count,local_heartbeat,service_count,flags,initialized,init_stage,init_error`。`init_stage` 使用 `DISTRIBUTED_REFMEM_INIT_STAGE_*` 固定枚举，`init_error=0` 表示初始化无错误。 |
...
| `SYSTem:REFMEM:LOAD:BOARD <board_id>,<board_uuid_crc32>,<capability_mask>,<io_constraint_mask>,<ip_core_mask>,<default_persona_mask>,<hw_profile_crc32>,<active_default_slot>,<online_required>` | 通过 SCPI 提交一条 BoardCapability 候选到 RefMem staging，用于调试、System Pack 前置验证和板卡能力实例化；必须包含 `REFMEM+VDC+TDMA` baseline，且只进入 staging，不直接修改 active BoardCapabilityTable。SCPI 只提交 intent，`DistributedRefMemAO` 通过 `BOARD_CAPABILITY_STAGE` command slot 写 staging 并 ACK/NACK；`board_id` 是 profile/candidate 标签，不是 A0-A7 target slot。返回 `STAGED/REJECTED` 加 board load snapshot。 |
...
| `REALtime:IO:ANALyzer:TRIGger:ARM <source_mask>,<sample_period_ns>,<max_records>,<timeout_us>,<overwrite_oldest>,<pre_records>,<post_records>,<trigger_type>,<trigger_source_mask>,<match_mask>,<pattern_value>` | 提交 `TRIGGERED_CAPTURE` 异步 ARM 意图。`trigger_type` 使用 analyzer trigger enum 的 LEVEL/EDGE/PATTERN；LEVEL/EDGE 的 `match_mask` 分别表示有效电平/变化位，PATTERN 使用 `match_mask` 与 `pattern_value`。Core1 只保留 bounded 前窗口、触发记录和后窗口；未触发到期以 `END_TIMEOUT` 收尾。 |
| `REALtime:IO:ANALyzer:STOP` | 提交异步 STOP 意图；返回 `1` 仅表示 mailbox accepted，Core1 完成硬件停止和 persona release 后由 `STATe?` 确认 request/handled/result 与资源归零；结束后的 capture 字段由 shadow snapshot 保留到 Core0 drain 完成。 |
