...
| `SYSTem:SYNC:VDC:OBServer:TDMA:SELFtest [role],[output_index],[observed_mask],[initial_sample_mask],[sample_period_ns],[pulse_period_ns],[pulse_high_ns],[pulse_count],[frame_crc32],[start_delay_ns]` | 维护态 TDMA observer 两板 bring-up：RX 角色按 active observation window 周期性启动 observer/capture，TX 角色用主输出组 PIO/DMA pulse train 产生参考边沿。`start_delay_ns` 让两板在共同未来窗口起跑，缺省为 `1000000000`。返回 `1`，不写 lock/offset/rate。 |
