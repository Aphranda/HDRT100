...
Status: Active
Domain: VDC
Canonical: `docs/vdc/VDC_RUNTIME_CONSTRAINTS.md`
Related: `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md`, `docs/vdc/VDC_DOMAIN_TODO.md`, `docs/check/DOCS_REGISTRY.md`
Last updated: 2026-09-19

> 分支范围：本文及“已登记”描述来自real-flight的`7a3e0954`，本次仅同步参考，
> 不新增或激活本地契约。详见[同步说明](../README.md#序列分支的上游文档同步范围)。

本文承接 [VDC 架构](VDC_DOMAIN_ARCHITECTURE.md)的已登记条款细则，保存编码 ABI、身份、取消、并发、算术和版本兼容边界。契约入口仍在主架构，ID、版本及状态以登记表为准；本次拆分不新增契约，不授予产品资格。

## 阅读范围

| 路线 | 适用范围 |
|---|---|
| 当前 typed 主线 | Core1 参考编码、直接 RX、MATCH/FOLLOW/PHASE、committed DCO、SYNC_IO 输出及内部探针。模块图与完成边界见主架构。 |
| 当前时基 | TX/MATCH 使用 `vdc_dpll_manager_project_timer1_feedback_event()`；RUN 使用 `vdc_timer1_coordinate.h`。主架构所述 TIMER1 直接坐标是当前实时路径。 |
| 旧桥接/历史记录 | 下文保留的 TIMER0/TIMER1 半开求交和粗阶梯说明用于原有 helper、旧 schema 与历史重放；不表示当前 typed TX/MATCH 仍经过 TIMER0 缓存。新记录依各自版本和真实调用路径解释，禁止混轴或放宽旧 decoder。 |
| 兼容分片与命令 | REFERENCE 和 BOUNDARY 是保留的独立模式，不是 typed 本地跟踪的前置。旧 FOLLOWER command 规则只在对应模式适用。 |
| 完成度 | 规范约束与已完成验收分开；“另行验收”等文字不提供即时任务状态，当前状态见 TODO，原始证据见 Task Progress。 |

已登记条款的通用不变量继续有效。当前 TIMER1 直接坐标只替代旧桥接路径的前提与计算，不取消事件区间、模型生效边界、身份/年龄、STOP 和溢出检查。旧 ABI/记录公式按原 schema 保留，不能用新路径重解释历史原件。

以下条款正文从主架构原样拆出，避免精简时丢失 wire、生命周期和异常路径约束；主架构保留同名入口并链接本文件。
...
origin 计数必须属于 capture 的 sync generation。新 TX owner 尚未初始化时，
上一代快照不成为当前基线；此前未取得当前代基线时，首次观察新代 owner 按其
零初始化累计本代拒绝；开窗时已取得当前代快照则只累计基线之后的增量。
已观察当前代后出现异代快照、或快照读取失败仍记录 SERVICE_GAP；同代计数回退
仍记录 COUNTER_RESET。此限定不忽略首段、空段或停止尾段，也不更改旧原生证据。
...
Core0 周期释放检查先只读预检已发布的后端退休状态，活动输出不触碰客户端门控。
预检只用于跳过工作；真正释放仍须一次弱 CAS 取得门控，并复验同一请求、后端
代际及退休状态。暂忙或读取失败留待后续独立 service，不能用锁外旧快照授权释放。
