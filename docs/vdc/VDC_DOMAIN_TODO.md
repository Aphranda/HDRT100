...
继续 `VDC-TIME-002`，接独立的所选几何观察器预启动；
不得恢复或假置 live alignment。预启动还须维护 START 前真实 DMA 计数的有界观察，
不能通过重设 epoch、主机短等待或零进度假设保持绑定。首验使用普通 origin 的限发
单帧，确认新 ARM 下的首条原始记录及拒绝结果；单帧不关闭连续性或物理身份门禁。
通过后再独立拆自主 origin 的非发射准备与显式首发，保留许可证到期、取消和配置
复验；不降低身份、回绕、故障退休和严格序列连续性要求。证据入口见
`VDC-PROGRESS-20260915-041`。当前四板先
