...
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。当前只准备共用输出投影，未接反馈运输或控制。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；正式反馈/命令接线前闭合 `VDC-FLIGHT-001` 的精确版本关联及本切片身份、时间关联、应用边界审核。 |
...
`VDC-FLIGHT-001` 保持 IN PROGRESS，下一步准备反馈与专属校正方案；正式接线仍按
