...
提供有效时间输入，三从没有合格输入。当前先贯通指定主机的特等席数据接收，随后
建立各从误差反馈和专属校正；该时间输入缺口保留为时间关联与精度优化任务。
...
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。原始反馈运输、稀疏主机参考配对、Core0 准备迁移及重臂对账见 `VDC-PROGRESS-20260916-009/010/011`；关联已提交 DCO 的模型反馈运输/配对见 `VDC-PROGRESS-20260916-012`。后继接通逐从校正和连续重基应用；当前未接控制，DPLL 及完整调度预算未闭合。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；正式命令接线前闭合 `VDC-FLIGHT-001` 的精确版本关联及本切片身份、时间关联、应用边界审核。 |
...
Domain 连续重基调频原语已独立验收，见 `VDC-PROGRESS-20260916-013`；当前尚无
生产调用。下一切片接入逐从服务边界命令、Core1 实际应用和精确反馈确认，先以
受限单次诊断校正验证链路，再评估稳态窗口的自动频率控制；不以原语测试关闭父任务。
反馈分片存在交付延迟，缓存覆盖、测量年龄和准备开销均须实测；DPLL 局部预算问题
单独优化，不新增健康 TDMA 隔离，也不以诊断功能通过宣称时序达标。
采样位置对照和
启用时间包围区间不替代输出域残差验证；不能把诊断历史标志直接改为 DPLL 合格输入。
`VDC-FLIGHT-001` 保持 IN PROGRESS，下一步推进反馈与专属校正接线；正式接线仍按
各切片依赖验收。长期目标不能因接收通过而关闭。
