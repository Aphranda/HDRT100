...
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-020外部全边沿约束与采集启动拒绝)及
...
下一步细分启动拒绝并恢复可重复采集，再做与模型/时间窗绑定的单因素纠偏。
...
1. 阅读本节和 `VDC-PROGRESS-20260917-019/020`；检查 `git status` 与实际 diff，
