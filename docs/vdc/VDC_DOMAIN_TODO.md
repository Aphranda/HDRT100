...
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-024映射约束保留上限实板收窄与外部复测)及
...
当前下一切片处理 DPLL 基线质量：初始远端区间约 1.6 µs，仍保留进后档，
而后续区间已显著更窄；原映射收窄不会追改旧基线。现有原件及真实 C helper
已核对：采用后续窄基线，对同一已记录当前 MATCH，NO2/NO3 的候选区间可从
跨零变为确定偏慢（离线反事实，未改变固件或预测控制轨迹）。下一步做有界、
模型/代际一致的基线选取/保留策略切片；不再以追加同类观测作为设计前置。不得伪造历史区间，
不重复本次上限/响应比例试验，也不重新加入 TDMA 首帧优化或重复校准前置。
...
| 当前下一步 | 收敛 `VDC-FAST-003`，IN PROGRESS | 进度 024 已验收映射上限及 trace 兼容、两段原生和外部波形；实际区间收窄但仍有漂移。宽基线贡献和窄基线候选已用原件及真实 C helper 定量验证，接着实现并验收有界 DPLL 基线质量策略，保持真实整数取整、模型/代际和远端界。VDC 隔离、observer 服务超预算及质量失败独立跟踪，不宣称全负载恢复、实时换模型或锁相；原失败保留。 |
