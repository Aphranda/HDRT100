...
| `VDC-LOCAL-003` | 从板加本地 delay 并接通本地跟踪 | IN PROGRESS | 依赖 `VDC-LOCAL-002`；Core0 同事件配对和 Core1 本地 DCO 已集成，等待时间基及桥接窗口优化完成源码/Release/直接四板 TDMA，见 `VDC-PROGRESS-20260916-032/033/034`。NO4 已有一次原生记录与模型一致的本地采用；NO2/NO3 当轮无完整参考，三从持续采用未完成。下一步用 owner 发布的停止态诊断定位分片运输和配对缺口，不截宽或强制取中点。复用已测矩阵，NO1 时间戳加有向 delay；Core1 独占 DCO，集中式控制互斥。当前源码 quick P3 待补；完整 bias/相位精度不作基础调频前置。 |
