...
| `VDC-LOCAL-003` | 从板加本地 delay 并接通本地跟踪 | IN PROGRESS | 依赖 `VDC-LOCAL-002`；Core0 同事件配对和 Core1 本地 DCO 已集成，等待时间基、桥接窗口及参考发布节奏切片已通过对应源码 quick 四板 P3，见 `VDC-PROGRESS-20260916-032/033/034/035/036/037`。最新两轮三从均收到完整参考并返回匹配 ACK，NO2/NO3 已进入本地控制判断；误差区间跨零时保持，不能把零步长记录当 DCO 采用。NO4 非零采用已复现，三从持续采用仍未完成。下一步缩窄真实观测不确定度并提高新鲜同事件配对连续性，先核对发布 OSAL 时基与物理时间的关系；保留片序、年龄、历史容量和 STOP 取消约束。复用已测线序及矩阵，不截宽或强制取中点；NO1 时间戳加有向 delay，Core1 独占 DCO，集中式控制互斥。完整 bias/相位精度不作基础调频前置。 |
