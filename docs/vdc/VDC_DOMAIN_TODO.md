...
[当前配置与主线接续](VDC_TASK_PROGRESS.md#vdc-progress-20260917-028独立输出-delay-持久化与本地相位提交)及
...
以当前频差水平直接推进实际输出 **100 ns 量级**锁相。这两项为用户目标，
不是当前已达性能或新增 P3 门槛；先接通相位闭环，再分别验证稳态相位误差与频差。
...
当前接续点为 `VDC-OUTPUT-001`：让 RUN 输出 owner 锁存 delay 并采用共同
未来 VDC 边沿，再测 NO1 相对三从的实际边沿差。不能将坐标平移或保存成功
...
| 当前下一步 | 推进 `VDC-FAST-003` / `VDC-OUTPUT-001`，IN PROGRESS / PENDING | 本地相位提交、频率估计共存及独立输出 delay 保存切片已完成，见进度 028；下一步接通实际 RUN 输出 owner 在确定边界采用共同未来 VDC 边沿及启动锁存的 delay。频差接近零不是前置。VDC 隔离、observer 服务超预算及质量失败独立跟踪，不宣称全参数开放、全负载恢复或锁相。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-028`；检查 `git status` 与实际 diff，
...
| `VDC-OUTPUT-001` | 实际 RUN 输出采用共同未来 VDC 边沿与独立补偿 | PENDING | 输出 owner 在启动时锁存 delay，转换共同目标为本地硬件时刻，使用持续 PIO/DMA 时间轴；已装入 FIFO 的边沿不可修改，迟到/断流/STOP 有界退休。以 NO1 为参考测量同周期上升沿差、斜率及抖动。配置可读写不等于物理补偿已生效。 |
