...
| `VDC-LOCAL-003` | 从板加本地 delay 并接通本地跟踪 | IN PROGRESS | 依赖 `VDC-LOCAL-002`；历史与 TIMER1 锚点的 TDMA owner 有界接口已完成源码/Release/直接四板 TDMA 回归，见 `VDC-PROGRESS-20260916-031`，静态 RAM 无增量；生产 Core0 缓存消费、同事件配对和本地 DCO 仍待接通。复用已测矩阵，形成 NO1 时间戳加有向 delay 的参考；显式控制模式下由 Core1 唯一 owner 更新，集中式控制不得同时写 DCO。完整 bias/相位精度不作基础调频前置；三从尚无本地跟踪更新证据。 |
