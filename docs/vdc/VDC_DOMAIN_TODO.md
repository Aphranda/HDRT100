...
RAM。评审方向已转入 [REFMEM 执行待办](../refmem/REFMEM_DOMAIN_TODO.md#当前优先插入任务refmem-静态缩容)。
RefMem 缩容、兼容迁移及本轮快速 P3 调试验收已完成，Release RAM 收益和四板布局专项均已核对；
严格质量检查仍有运输/调度失败，保留在主线处理，不再反向阻塞 RAM 切片结项。当前部署状态以 `REFMEM-TASK-20260916-001` 为准；
本轮读回后四板均 STOP，RAM 不再叠加改动，接下来回到 `VDC-FAST-002` 修复共同失败。
...
| 当前下一步 | 收敛 `VDC-FAST-002`，IN PROGRESS | 继续同一快速入口失败切片；优先核对板端 Flash/XIP 实际读取配置，再按实测选择复制、校验或发布的一项优化。确认四板 process-image 启动及持续收发恢复，并通过调度门禁；不以 priority 槽位 CRC 通过代替整体运输。 |
| 随后 | 完成 `VDC-FAST-001/002` | 最小预编码同步记录在固定位置装载、飞行转发、本圈保全并进入目标 Core1；配置、delay 和上下文提前安装，无普通解析、RefMem 分片或 RTOS 前置。 |
...
1. 阅读本节、`VDC-PROGRESS-20260916-043/042` 和 RefMem 当前插入任务；检查 `git status`
   与实际 diff，保留另一设备合入的单板工作及后继改动。当前代码基线已提交为 `e6ff06a`，
   暂存源码指纹与分级 quick P3 匹配；严格质量失败仍不能因进程退出为零而被追认通过。
2. 以进度记录链接的 `measured-build-r5-identified/`、`pre-p3-r5-manifest.json`、
   `p3-r5/acceptance.json` 和 `independent-review/hardware-r5-review-r2.json`
   为缩容前基线；后者已纠正早期摘要，明确启动、持续运输和调度均未通过。
3. RefMem 缩容的兼容测试、Release/map、四板 quick P3 与专项读回已执行；接续时
   复用进度中的证据，保留严格失败，不重复缩容或追认通过。已知线序和 delay 继续复用，不重跑 P0T，不修改 OTA。
4. 返回快速通道，逐项解决共同运输/调度失败；每个功能切片独立测试、构建、四板 P3及专项。
   下一诊断候选详见 `VDC-PROGRESS-20260916-042` 的 XIP 只读移交，不同时叠加多项优化。

边界保持：Core1 不动态分配、不等待 Core0、不调用存储/SCPI/RTOS；TDMA 独占 PIO/DMA，
DATA FIFO 仍只有 DMA 读取。SCPI 只触发运行流程，板端记录、全部 STOP 后读取。
示波器接线和使用条件沿用既有记录，精度验收前重新核对；本轮未做新的示波器采样。
...
最新验证状态由 `VDC-PROGRESS-20260917-001` 承接，固件主线方向不变。
...
| `VDC-FAST-002` | IN PROGRESS | 接入预编码时间戳的确定性装载与独立接收保全。硬件/TDMA owner 按事件和授权边界填充、选择固定记录，不依赖 RTOS 每片发布或普通 overlay worker 每事件重建；在通用 RX 工位和可丢弃镜像 FIFO 之前保全记录。证明 DMA 覆盖期限、固定队列容量与消费能力；缺口、无效和溢出显式记录，过期数据不得伪装为新样本。每个实现切片单独通过软件、资源、当前源码四板 quick P3 和运输专项。 | `VDC-FAST-001`；当前先实现复用现有邮箱格式的独立原始接收入口，栈修复、空通道调度收益及 IRQ 分段实测见 `VDC-PROGRESS-20260916-040/041/042`，运输及调度门禁仍未通过。紧凑 wire/资源契约变更仍须先完成相应登记及审核。 |
| `VDC-FAST-003` | PENDING | 将最小同事件配对接入 Core1/SyncDpllFB：按固定偏移读取已编码字段，以事件序号直接索引本地记录并复验完整序号/代际，再执行有界差值、delay 与控制更新；不遍历历史、不等待 Core0、不进行通用分片重组。Core0 保留配置、低频元数据和诊断；迁移缓存明确唯一 writer。实测新增及完整 Core1 WCET，验证重复、缺失、回绕、代际切换、STOP/ARM，并以三从实际 DCO 与 internal 残差闭环验收。 | `VDC-FAST-002`；固定编码仍须有可核验的完整性和来源准入。 |
