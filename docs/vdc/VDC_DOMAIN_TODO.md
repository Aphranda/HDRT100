...
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-021固定输出快照准入与轻量时钟资格)及
...
候选为仅调整 `priority_follow_delta()` 最近零边界的响应比例，保持区间、窗口、
NO1 PI、准入和饱和规则不变；先离线与 host 验证，再独立四板 P3 和新鲜波形。
候选尚未实施，不因外部偏慢直接判定增益缺陷；不同时间窗的内部 secant 和
停止后固定模型频差不能当成同一时刻的误差，必要时先延长同控制运行作基线。
...
| 当前下一步 | 收敛 `VDC-FAST-003`，IN PROGRESS | typed 连续事件差分已在原生记录中验证收窄和真实采用，见进度 019。保持实际整数取整与远端界，对照外部剩余漂移、后档未决区间和 NO1 PI 参考继续频率收敛；不重复实现长窗口或本地新 helper。VDC 隔离、observer 服务超预算及本轮新增质量失败独立跟踪，不宣称全负载恢复、实时换模型或锁相；原失败保留。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-019/020/021`；检查 `git status` 与实际 diff，
...
最新两轮固定模型波形与剩余漂移见进度 021，未取得锁相证据。
...
最新验证状态由 `VDC-PROGRESS-20260917-021` 承接，固件主线方向不变。
