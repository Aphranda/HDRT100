...
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-018有界长窗口产生真实后档-dco-调整)及
...
| 当前下一步 | 收敛 `VDC-FAST-003`，IN PROGRESS | 有界长窗口已验证真实后档 DCO 调整，见进度 018。保持实际量化界，对照外部剩余漂移、后档未决区间和 NO1 PI 参考，继续频率收敛；不重复导入旧隔离候选。VDC 隔离与 observer 服务超预算独立跟踪，不宣称全负载恢复、实时换模型或锁相；STOP 后模型超时等原失败保留。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-017/018`；检查 `git status` 与实际 diff，
