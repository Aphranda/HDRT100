...
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-022外部反馈驱动的有界跟随响应试验)及
...
最新续跑中 NO1 参考明显变化，下一步先关联同窗参考轨迹与从板误差，继续核
远端编码宽度及响应延迟；不重复做此次响应比例切片，不盲目继续增益翻倍。
不同时间窗的内部 secant 与停止后固定模型频差不能当成同一时刻的误差。
现有 schema 2 ORIGIN 可记录 NO1 rate/DCO/源事件及编码；优先复用同窗有限
前缀，核满缓冲冻结及代际条件。当前原件只有 NO1 起止模型，不能重建中间
轨迹，也不能把有限前缀说成覆盖整个跟随窗口；先核可达范围，不新增遥测阻塞。
...
1. 阅读本节和 `VDC-PROGRESS-20260917-021/022`；检查 `git status` 与实际 diff，
