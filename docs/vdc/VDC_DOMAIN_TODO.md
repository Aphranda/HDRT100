...
`VDC-PROGRESS-20260915-031`。切换前已排队、切换后才首次接收的未来命令仍需入站
fence；远端代际重启、Domain 任意时钟模型换会话后的历史退休及完整 STOP 取消
仍待闭合。不能因禁用态 P3
