...
| `VDC-FEEDBACK-001` | PENDING | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。退出须逐从追溯测量身份、反馈、命令和实际应用。 | `VDC-FLIGHT-001`；本切片所需反馈/命令身份、时间关联及应用边界经审核。 |
