...
| 顺序 | 下一步 | 对应任务 / 退出条件 |
|---|---|---|
| 1 | 验证缺参考时模型保持、质量降级及恢复发布 | `VDC-SNAPSHOT-001`、`VDC-HOLD-001`；局部发布与外参恢复见 026–028，typed 新鲜度见 030，停更/再 ARM 见 031，Core0 诊断镜像见 032。先对账 032 首轮 GUARD 首次成功等待/稳态缺口和自主停机链（同配置复测通过），再推进正式质量及恢复逐事件证据；补偿 HOLD 与 Domain HOLDOVER 分开判定。 |
