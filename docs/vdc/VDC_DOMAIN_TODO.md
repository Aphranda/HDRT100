...
1. 按下方 `VDC-TIME-001` 至 `VDC-TIME-004` 补齐自主 origin 的实际事件、原始计时、
   记录生命周期及输入准入。事件/资源审计已有证据，当前实现入口是
   `VDC-TIME-002`；不能将 RTT 倒数或 CPU 提取时刻升级为边沿时间戳。
...
| `VDC-TIME-002` | 完成 TDMA owner 内的原始计时记录原型及资源收敛。 | IN PROGRESS | 依赖 `VDC-TIME-001`。真实 builder 集成完整捕获/发布图；全部准入节点数的描述符、literal、单步构造上界、记录布局、FIFO 和目标 RAM/link map 均通过。区分 timer 读取与实际边沿，记录不确定度；覆盖跨字回绕、缺边沿、旧 FIFO、身份错配、覆盖、STOP/重臂和有界取消。当前离线指令模型及操作成本探针只作设计证据，不能替代目标实现测试。 |
| `VDC-TIME-003` | 验收四板自主模式的原始计时与完整窗口连续性。 | PENDING | 依赖 `VDC-TIME-002`。当前源码构建/P3及四板有限许可证采集，原始记录按 epoch/sequence/identity 对账；约束实际边沿到计时的偏移和抖动，保留切换失败、缺失及迟到。SCPI 仅控制，全部 STOP 后顺序保存 TDMA/DPLL 并核对 CRC/SD；健康 TDMA 不因局部计时拒绝而隔离。 |
...
下一次实现先处理 `VDC-TIME-002` 的集成图和存储预算。不能只利用当前实接节点较少
而忽略已准入的更大节点配置，也不能关闭记录以掩盖新增计时的资源成本。
