...
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。输出投影、主机原始记录、独立采样配置、观察器自身恢复和同条观测留存已完成切片验收；原始反馈已完成当前源码四板运输与重臂对账，证据见 `VDC-PROGRESS-20260916-009`。后继接通主机参考关联、逐从校正和实际应用，当前未接控制。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；正式反馈/命令接线前闭合 `VDC-FLIGHT-001` 的精确版本关联及本切片身份、时间关联、应用边界审核。 |
...
`VDC-PROGRESS-20260916-009`，下一步建立同次主机参考、逐从校正和实际应用身份。
反馈分片存在交付延迟，主机参考缓存须按实测延迟评估，不能把最新单条记录当作历史。
采样位置对照和
