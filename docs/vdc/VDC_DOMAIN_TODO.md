...
首轮拓扑准备超时保留。后继优先复用现有 PIO、描述符和首档原件，证明唯一发车、
首 CS 内完整时钟数以及实际分频/同步相位下逐位采样一致，再合成首窗口不跨帧的
物理身份；不以固定整数周期模型代替实际分频边界，也不先扩充诊断缓冲。
当前继续物理身份关联；从板 TDMA 相位仍有实测超限，后续时序任务继续保留
全相位预算门禁，不以首窗口复制耗时或普通 P3 通过关闭自主调度失败。
继续 `VDC-TIME-002`，闭合同一物理帧的唯一 packet/event 坐标与完整自主窗口，
才开放 `VDC-TIME-003/004`。主板首档身份仍来自本地 TX header，不能由它与后续
正确帧的序列关系授予物理身份；不降低回绕、故障退休和严格序列连续性要求。
当前四板先
