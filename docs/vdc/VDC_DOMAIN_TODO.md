...
打通“从板观测 → NO1 逐从计算 → 专属命令运输 → 从板 Core1 连续应用 DCO →
实际应用反馈”的闭环。首先使 NO2–NO4 相对 NO1 的持续频率漂移收敛，再实现
...
| 里程碑 | 对应任务 | 必须看到的结果 |
|---|---|---|
| 命令真正生效 | `VDC-FLIGHT-001`、`VDC-FEEDBACK-001` | 每个从板的测量、专属命令、Core1 实际应用与回传 ACK 可以逐条对应；接收成功不能代替应用成功。 |
| 自动消除频偏 | `VDC-DRIFT-001` | 按各从自身反馈持续闭环，internal 输出域残差斜率收敛，并可与本板实际 DCO 更新对应；固定单次扰动只验执行链。 |
...
`VDC-PROGRESS-20260916-018`。TDMA 重配置的 servo CRC 缺陷修复后，同命令有界重复
已完成当前源码四板 P3、同轮全从板实际应用/ACK 专项及独立复核。
当前进入逐从自动频率控制的方案分析，尚未接入生产自动控制，不能由单次诊断
成功直接提升为自动闭环完成。
...
| `VDC-FEEDBACK-004` | 接入逐从自动频率控制 | IN PROGRESS | 当前仅方案分析，自动控制尚未接入。验证有效频偏估计的方向、量纲和窗口，分清可消除的固定锚点误差与逐次测量误差；各从独立控制状态、限幅、更新节奏及失效保持有据可查，再进入 `VDC-DRIFT-001` 的收敛验收。 |

### 当前主线：四板数据运输与真实闭环

在 HAOFV 架构下，通过 TDMA 飞行环路传递 DPLL/VDC 特等席数据，将 NO2–NO4 纳入
相对 NO1 的真实闭环：先接通数据和各从专属校正，消除持续频率漂移，再收敛相位误差。
当前用户目标快照为 **100 ns 量级**，非已实现精度或冻结门限；正式验收须事先明确
峰值/RMS 指标、稳定窗口和测量不确定度，不能用 timestamp resolution 门限替代输出误差。

| Task ID | 状态 | 工作与退出条件 | 依赖 |
|---|---|---|---|
| `VDC-FLIGHT-001` | IN PROGRESS | NO1 发布数据，各从保留指定主机的新数据；当前源码四板证明来源、序号、编码后数据一致及持续接收，其他来源不能覆盖。 | 现有四板 TDMA 通路；资源及有界交接复核。 |
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。原始反馈运输、稀疏主机参考配对、Core0 准备迁移及重臂对账见 `VDC-PROGRESS-20260916-009/010/011`；模型反馈运输/配对见 `VDC-PROGRESS-20260916-012`。逐从边界命令及连续重基应用的软件接线见 `VDC-PROGRESS-20260916-014`，同轮全从板单次实际应用/ACK 专项见 `VDC-PROGRESS-20260916-018`；自动控制、DPLL 及完整调度预算未闭合。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；命令运输和内部单次应用身份已有专项，自动控制仍须闭合新的测量窗口、估计及准入审核。 |
...
- TDMA Core1 独占 PIO/DMA、物理时序与运输；特等席使用固定配额和有界交接，
  Core0/RefMem 完成准备、解析、校验和稳定快照发布，不能让解析等待阻塞发车。
- Core1/SyncDpllFB 独占实时控制和 DCO 应用；主机按各从反馈生成专属校正，FOLLOWER
  保持既有角色，不隐式开启本地 PI。复制 NO1 自身补偿量只能提供前馈，不能闭合各从误差。
