...
主板短窗未再记录 TDMA 超限，但从板、整表及长期预算仍待闭合。下一步优先推进
事件 FEED 内计数回绕 lift 的等价快路径，保留多回绕歧义、错误原因和故障退休；
独立 P3 后复测，再决定后续 START_CUT 或代码放置优化。身份、数据准入和故障
复验不因降耗而省略。新增探针与应用槽位影响测量，继续控制配置和槽位评估收益。
