...
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-017调度入口迁入-sram-与外部剩余漂移复测)及
...
| 当前下一步 | 收敛 `VDC-FAST-003`，IN PROGRESS | 调度入口迁入主 SRAM 后已完成软件、资源、同源码四板 quick P3、两轮末段匹配及四路物理输出复测，见进度 017。保持实际量化界，对照外部剩余漂移、内部跨零保持和 NO1 PI 参考，再评估隔离且未部署的长窗口候选。候选 VDC 隔离与 observer 服务超预算独立跟踪，不宣称全负载恢复、唯一 XIP 因果、实时换模型或锁相；原失败保留。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-016/017`；检查 `git status` 与实际 diff，
