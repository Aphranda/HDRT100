...
## 下次接续入口：IRQ 已直接交接全部准入记录，继续同事件匹配与本地控制

本节为用户要求保留的执行断点；长期目标仍是 `VDC-LONGTERM-001`，不缩减为原始
邮箱接收或单次 DCO 更新。下一次“继续”从本节开始，详细实测见
[当前直接接收进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-005同步记录直接交接与-irq-热路径收敛)及
...
| 当前下一步 | 收敛 `VDC-FAST-001/002`，IN PROGRESS | NO1 直接发布与三从 IRQ 同步交接已验证；继续物理帧到入口期限及 IRQ 预算，安装接收端完整运行绑定与本地事件上下文。按 source event 直接索引，载荷圈仅作运输证据。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-003/004/005`；检查 `git status` 与实际 diff，
...
4. 在 IRQ 直接交接后的专用实时 handoff 中安装绑定；按完整源事件序号直接定位
   `TDMA_EVENT_HISTORY_CAPACITY` 内的一条历史，复验 epoch/ordinal/完整序号并借用
   同生命周期 TIMER1 enable bracket，不能扫描历史或借用 LIVE 的另一事件身份。
   本地已测 delay 加在远端参考区间以得到预期 RX 区间，实际 DCO 模型投影保留上下界；基础闭环
...
最新验证状态由 `VDC-PROGRESS-20260917-005` 承接，固件主线方向不变。
