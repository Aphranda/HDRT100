...
退休，历史候选不能作为当前 live lease。本轮唯一自主尝试在准入拒绝后提前停止，
部分窗口和严格拓扑失败原件保留，未形成自主模式匹配或完整窗口证据。
下一步沿准入诊断追踪拒绝条件，同时只读收敛物理首事件、packet/event 身份与 DMA
anchor 的证明边界；不得把候选 sequence 相等提升为正式时间戳。自主总门禁、身份
和物理精度仍未闭合，`VDC-TIME-003/004` 不开放。证据见
`VDC-PROGRESS-20260915-001/003/004/005/006/007/008/009`；OTA 保持现状。
