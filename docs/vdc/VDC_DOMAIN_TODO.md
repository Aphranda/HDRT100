...
| `VDC-LOCAL-003` | 从板加本地 delay 并接通本地跟踪 | IN PROGRESS | 依赖 `VDC-LOCAL-002`；Core0 同事件配对和 Core1 本地 DCO 已集成，等待时间基及桥接窗口优化、当前源码 quick 四板 P3 和提交收敛完成，见 `VDC-PROGRESS-20260916-032/033/034/035/036`。NO4 本地采用已在复位后复现；三从持续采用仍未完成。停止态证据把定位重点推进至 FIFO 前的物理观察遗漏；下一步验证 typed 分片驻留与两阶段 RX 观察间隔，并保留片序、年龄、历史容量和 STOP 取消约束。复用已测线序及矩阵，不截宽或强制取中点；NO1 时间戳加有向 delay，Core1 独占 DCO，集中式控制互斥。完整 bias/相位精度不作基础调频前置。 |
