...
基础已完成 source/host、目标资源与普通短帧验收；本切片自主采集在 grant 准入时
失败，先补齐拒绝 reason/阶段并复测完整窗口，不能把已救回的中止记录当作通过。
之后继续原始事件与已校验 packet 的有界身份候选关联：逐 capture lease 不复用 STOP 取消代际，
绑定 observer/ARM 代际及 DMA 复制范围，明确覆盖、迟到、错序与取消退休。候选仍为
诊断属性，物理 anchor 和正式资格随后单独验收。自主总门禁、身份和物理精度仍未
闭合；严格校准失败保留。证据见 `VDC-PROGRESS-20260915-001/003/004/005/006`；OTA 保持现状。
