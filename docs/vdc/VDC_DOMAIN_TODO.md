...
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。已准备输出投影、主机原始记录及独立采样配置；观察器自身恢复取得四板专项证据，当前收敛复核并准备同条观测留存与反馈运输，未接控制。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；正式反馈/命令接线前闭合 `VDC-FLIGHT-001` 的精确版本关联及本切片身份、时间关联、应用边界审核。 |
...
保留各从同次观测及本板时钟锚，建立反馈运输和实际应用身份。采样位置对照
