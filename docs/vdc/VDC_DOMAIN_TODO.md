...
## 下次接续入口：typed 本地 DCO 已接通，观测漂移与估计不确定性

本节为用户要求保留的执行断点；长期目标仍是 `VDC-LONGTERM-001`，不缩减为原始
邮箱接收或单次 DCO 更新。下一次“继续”从本节开始，详细实测见
[当前观测进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-009诊断输出扰动示波器窗口失败与静默恢复)及
...
互斥 typed 本地频率估计与真实 DCO 应用已通过同源码 quick P3 和短时专项：
NO4 已真实采用修正，NO2/NO3 的宽区间跨零而保持零调整；不能据此认定无需同步。
下一步观测实际漂移并收窄估计区间。保留溢出/epoch 失效，不清 sticky 后冒充同一时间轴；
逐帧必达、GPIO 模型采用、斜率收敛与锁相仍待证明，调度预算和长时间连续性继续跟踪。
后继开启示波器诊断输出期间发生观察器 DMA 环覆盖与跟踪取消，关闭该输出后静默复测恢复通过。
因此先为 typed 路径补有界 SRAM 原生记录，直接保留残差区间与实际 DCO；
旧 internal 更新门控和现有稀疏诊断脉冲均不能替代这项记录，详见最新进度。
...
| 当前下一步 | 收敛 `VDC-FAST-003`，IN PROGRESS | typed 本地频率控制已通过短时专项；接入有界 SRAM 原生记录，STOP 后读取残差区间与实际 DCO，再分析跨零不确定性与漂移。当前有扰动的诊断输出暂不作为测量入口，GPIO 采用及物理精度后继独立验证。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-009`；检查 `git status` 与实际 diff，
...
5. typed 本地 DCO 已从事件间隔计算 ppb 区间并接入 Domain，不能将 ns 直接当 ppb，
   不伪造远端 model token。先做漂移观测；旧 internal capture 的 `dpll.update_seq` 门控
...
示波器接线和使用条件沿用既有记录；最新短窗采集失败与静默恢复见进度 009，未取得锁相证据。
...
最新验证状态由 `VDC-PROGRESS-20260917-009` 承接，固件主线方向不变。
