...
同槽复测未证明从板整体提速；保留算术简化和代码缩小，不关闭时序任务。下一步
分解 START_CUT 的持续复验和 Flash 执行开销，再选择独立优化；保留多回绕歧义、
错误原因、身份及已退休后的故障累计。新增探针与应用槽位影响测量，继续控制配置
和槽位评估收益。完成时间输入/命令应用后，以 NO1/CH1 上升沿触发的四路示波器
采集与板端记录共同判断锁相，不以 TDMA 短窗通过替代输出证据。
