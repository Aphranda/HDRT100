...
[当前配置与主线接续](VDC_TASK_PROGRESS.md#vdc-progress-20260918-003相位内缓存交接与-sram-热路径验证)及
...
长时间轴参数按用户要求纳入后继实现：提前规划时长、硬件承诺时长上限、补给
低水位均以时间量表达，通过 STOP-only SCPI 修改，显式 STORe 固化 Flash，
启动恢复请求值并在下一 ARM 锁存。Core1 仅使用已安装参数，不在 RUN 读取 Flash
或依赖 SCPI；具体命令、默认值和合法范围待连续时间轴实现及目标板预算确定，
当前尚未实现这些新参数。联合校验须覆盖缓冲容量、补给服务间隔/耗时、硬件
库存和 DPLL 修正延迟；旧持久化记录兼容默认值、保存/重启/回读和非法配置拒绝
列为该切片验收项，不修改 OTA。新增参数不能只保存而不进入真实计划和补给路径。
...
| 当前下一步 | 推进 `VDC-FAST-003` / `VDC-OUTPUT-001`，IN PROGRESS | 进度 `VDC-PROGRESS-20260918-003` 已验证缓存交接入口及 SRAM 热路径，本次短服务样本符合候选预算，输出仍 STARVED。接续连续硬件长时间轴、提前规划和按剩余执行时间补给；区分可修改软件后缀与硬件承诺库存，兼顾服务空窗和 DPLL 更新延迟，隔离块界重投影/模型连续性。保留 NO1 PI，不宣称锁相。 |
...
| `VDC-OUTPUT-001` | 实际 RUN 输出采用共同未来 VDC 边沿与独立补偿 | IN PROGRESS | 有限持续输出、资源预留、完整客户端交接及不可改写前缀已实现；进度 030/031 保留低频通过、断流和诊断。进度 `VDC-PROGRESS-20260918-001/002` 后缀预规划及自主周期释放完成，仍需修复服务空窗并分离块间桥接投影/模型连续性，再验证相对 NO1 的同序边沿差、斜率及抖动；计数和多脉冲可见不能当精度已达。 |
