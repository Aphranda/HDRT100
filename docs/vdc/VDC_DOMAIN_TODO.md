...
## 下次接续入口：P0 修复验收后继续特等席同步发布

本节为用户要求保留的执行断点；长期目标仍是 `VDC-LONGTERM-001`，不缩减为原始
邮箱接收或单次 DCO 更新。下一次“继续”从本节开始，详细实测见
[当前 P0 与同步编码进度](VDC_TASK_PROGRESS.md#vdc-progress-20260917-003p0-瞬态拒绝恢复与-typed-同步接收切片)及
...
当前已增加独立 typed 同步类、定长 body 编解码和 Core1 有界解码；它仍只保留记录，
最小同步编码的确定性发送、Core1 事件匹配及该路径上的实际 DCO 更新尚未接通。
编码时间单位、绑定代际的安装/失效语义及完整邮箱验证仍须随发布端闭合，不能宣布编码方案已全部完成。
...
定段修复验证已完成，当前匹配源码四板 P3 结果见最新进度；固件内部具体拒绝分支尚未定位。
RAM 不再叠加改动，P0 切片闭合后回到 `VDC-FAST-001/002` 的同步发送。
...
| 当前下一步 | 收敛 `VDC-FAST-001/002`，IN PROGRESS | 完成 P0 修复和 typed 解码切片的匹配源码四板验收，再接 NO1 确定性发布；明确时间单位、绑定代际、有效性和失效行为。body 解码不授予 DCO 权限，旧普通类不得重新解释为同步记录。 |
| 随后 | 完成 `VDC-FAST-001/002` | 最小预编码同步记录在固定位置装载、飞行转发、本圈保全并进入目标 Core1；配置、delay 和上下文提前安装，无普通解析、RefMem 分片或 RTOS 前置；分别测量运输、事件年龄与控制延迟。 |
...
1. 阅读本节和 `VDC-PROGRESS-20260917-002/003`；检查 `git status` 与实际 diff，
...
4. 返回快速通道，接 NO1 同步发布，再接 Core1 同事件匹配与本地 DCO；
...
最新验证状态由 `VDC-PROGRESS-20260917-003` 承接，固件主线方向不变。
