...
| `VDC-LOCAL-003` | 从板加本地 delay 并接通本地跟踪 | IN PROGRESS | 依赖 `VDC-LOCAL-002`；Core0 历史消费、同事件配对和 Core1 本地 DCO 入口已集成，等待参考时间基错配已修复，源码/Release/直接四板 TDMA 通过，见 `VDC-PROGRESS-20260916-032/033`；实板三从仍无实际采用。下一步用停止态诊断定位完整参考稀疏与配对缺口，独立缩短桥接投影采样区间，不截宽或强制取中点。复用已测矩阵，NO1 时间戳加有向 delay；显式模式下 Core1 唯一 owner 更新，集中式控制不得同时写 DCO。完整 bias/相位精度不作基础调频前置。 |
