...
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。原始反馈运输、稀疏主机参考配对及重臂对账已完成切片验收，证据见 `VDC-PROGRESS-20260916-009/010`。后继降低实时准备成本、关联实际输出模型，接通逐从校正和实际应用；当前未接控制，DPLL 及完整调度预算未闭合。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；正式反馈/命令接线前闭合 `VDC-FLIGHT-001` 的精确版本关联及本切片身份、时间关联、应用边界审核。 |
...
下一步迁移准备成本时须冻结跨核硬件来源和取消代际，再建立实际输出模型关联、
逐从校正和实际应用身份。原始差分未包含实际 DCO 模型，不是输出域残差。
