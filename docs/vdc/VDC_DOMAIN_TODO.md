...
最新证据见 `VDC-PROGRESS-20260918-044`；不继续扩大旧
...
区间中位由 324 ns 降至 192 ns（有限实测快照，非精度契约）。最新一次外部边沿
中位约为 NO2/NO3/NO4 相对 NO1 的 +120/+175/+240 ns；另一轮仍处于明显调节，
...
| 当前下一步 | `VDC-OUTPUT-001`，IN PROGRESS | TIMER1 迁移切片已验收，见进度 040/041。STOP 配置的 output delay 在 RUN 原生读回中生效，但零值及相同候选的跨启动物理偏差仍不可重复。先对账主从模型相位锚与 RUN enable 映射，再做固定条件下的输出 delay 响应验证；path delay 单独处理。NO1 OUT4 已接 EXT，但 RUN 当前仅 OUT1，继续 CH1 触发。 |
...
| `VDC-OUTPUT-001` | 实际 RUN 输出采用共同未来 VDC 边沿与独立补偿 | IN PROGRESS | 有限持续输出、资源预留、客户端交接、后缀预规划与不可改写前缀已实现；TIMER1 坐标迁移已验收。进度 `VDC-PROGRESS-20260918-041` 保留零值/候选/恢复对照及适配器失败；下一步对账跨启动模型相位与 RUN 映射，证明 output delay 可重复响应，再验证相对 NO1 的同序边沿差、斜率及抖动。最近边沿中位及计数不替代精度验收。 |
