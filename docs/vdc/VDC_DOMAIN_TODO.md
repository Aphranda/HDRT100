...
下一步先通过 owner 有限发帧或等效最早帧保留补齐首帧 DATA、CS/SCK 与取消证据，
再推进冻结几何和 observer 预启动；不以主机等待时间或较晚的正确帧授予零起点。
下一步排除在途 DMA 写入、启用前积压并证明首物理边界，建立唯一 packet/event 坐标；
FIFO 空、两端计数相等或候选 sequence 相等均不能授予身份。011 阶段的严格 P3
通过不追认历史失败；013 当前仅 quick 通过，严格校准失败另行保留。自主 startup、
普通计数/persona 和调度超限继续分别追踪。
自主总门禁、身份
和物理精度仍未闭合，`VDC-TIME-003/004` 不开放。证据见
`VDC-PROGRESS-20260915-001/003/004/005/006/007/008/009/010/011/012/013`；OTA 保持现状。
