...
| 当前下一步 | 推进 `VDC-FAST-003` / `VDC-OUTPUT-001`，IN PROGRESS | `VDC-PROGRESS-20260918-006` 已完成 schema7 服务序号/缓存失效/退休 tick 相关采样。四板仍观测到 `TXSTALL + FIFO empty + DMA remaining=0`；NO1 的一次 `submit_rejected` 与末沿耗尽已建立先后关系，但内部拒绝原因尚未分解。下一步细分提交拒绝或验证私有后缀重试，区分服务空窗、模型重建和 DMA 准入；保留 STARVED 事实和 NO1 PI，不宣称连续性或锁相。 |
