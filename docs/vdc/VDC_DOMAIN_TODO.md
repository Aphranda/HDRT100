...
| `VDC-FEEDBACK-001` | IN PROGRESS | 各从 internal 有效观测经 TDMA 回到 NO1，主机按每个从板的误差维护独立控制状态、生成对应命令；各板 SyncDpllFB 实际应用 DCO。已准备输出投影、主机原始记录及独立采样配置；当前先修复启动重复序号后的观察器自身恢复，未接反馈运输或控制。退出须逐从追溯测量身份、反馈、命令和实际应用。 | 已验收的指定主机接收切片支持本地准备；正式反馈/命令接线前闭合 `VDC-FLIGHT-001` 的精确版本关联及本切片身份、时间关联、应用边界审核。 |
...
帧头对照通过，序号专项暴露启动重复序号使旧观察器永久退休。先实现观察器自身
有界恢复，再保留各从同次观测及本板时钟锚，建立反馈运输和实际应用身份。采样位置对照
不替代输出域残差验证；不能把诊断历史标志直接改为 DPLL 合格输入。
