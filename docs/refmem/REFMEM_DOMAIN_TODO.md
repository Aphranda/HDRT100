...
- [ ] 实现自组网协调消息：`CLAIM_HELLO`、`CLAIM_PROPOSE`、`CLAIM_CONFLICT`、`CLAIM_RELEASE`、`CLAIM_RESOLVE`、`CLAIM_COMMIT`；当前已落地 `CLAIM_HELLO`、`CLAIM_PROPOSE`、`CLAIM_COMMIT` 固定 frame header、payload CRC、header CRC 和纯 C 单元测试，尚未接入 RJ45 发送/接收。
