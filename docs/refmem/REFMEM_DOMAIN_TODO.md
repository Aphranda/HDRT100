...
- [ ] P0: 风险评审 §3.11 `SYSTem:REFMEM:LOAD:NODE` / `LOAD:BOARD` staging 不能停在 metadata-only 死胡同；`LOAD:NODE` 已形成私有 NodeLoadTable staging image，`LOAD:BOARD` 已形成私有 BoardCapabilityTable staging image 并发布整表 CRC。剩余闭环：inline staging 必须生成可 activation、可 rollback、可 runtime parse 的完整 package image，或明确降级为不参与 activation 的诊断入口。
