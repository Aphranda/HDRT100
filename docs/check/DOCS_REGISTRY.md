...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱与运行代际；Core1 编码/直接匹配、TDMA 双缓冲及 RX IRQ 交接；NO1 与从板 MATCH 独立有限共钟半开求交、最终准入后提交，origin 编码与原生重放；typed 共钟差分与旧阶梯界分离，有界窄基线与档位复评；STOP 配置、绑定锁存及显式 Flash 保存；可选本地相位精确平移，以完整发布回执授权同 rate epoch 的累计平移归一化，频率更新/未知模型/STOP 取消；独立输出 delay 与整数反解/共同网格；SYNC_IO 有限持续 PIO 输出、STOP 资源预留、完整客户端交接、START/模型身份准入、不可改写前缀、保守 raw/enable 锚与异步退休；可配置时间窗口、有限固定映射与新鲜 raw 准入、分批后缀规划及调度容量校验；固定高宽 STOP 预装 ISR、单字低段编码及逐块宽度复验，保留双字能力；不授予产品 RUN 或锁相 | 16 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_match.inc | codec/绑定/STOP/直接入口/历史/共钟及 Domain oracle、频率/相位共存、输出生命周期/实际 PIO 编码/故障注入、参数迁移/Flash 恢复/时间轴回绕、有限 FIFO/模式切换、目标资源、同源码四板 P3 与专项 | 2026-09-18 | pending |
