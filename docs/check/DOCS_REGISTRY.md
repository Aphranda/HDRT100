...
| TDMA-FLIGHTBITMAP-01 | tdma | SHORT mailbox 数随 PROJECT_NODE_CAPACITY 编译，单邮箱布局与 core1 RX 位图固定；STOP 后按拓扑选择布局为后续候选，RUN 长度不得变化 | 2 | docs/tdma/TDMA_DOMAIN_ARCHITECTURE.md | tdma_flight_engine.h | 容量矩阵、异长度拒绝、DMA/overlay 单测与多板 HIL | 2026-08-21 | pending |
