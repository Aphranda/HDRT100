...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱、output-ns 区间与非复用运行代际；Core1 编码、TDMA 双缓冲及 RX IRQ 交接；直接索引源事件、复验生命周期并形成带暂定定向 delay 的残差，不授予 DCO/锁相 | 3 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_match.inc | codec/运行绑定/STOP/直接入口/精确历史/区间算术 host、目标资源、匹配源码四板 P3 与匹配专项 | 2026-09-17 | pending |
