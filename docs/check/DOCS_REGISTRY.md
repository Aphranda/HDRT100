...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱、output-ns 区间与非复用运行代际；Core1 编码、TDMA 双缓冲及 RX IRQ 交接；源事件直接匹配，显式互斥本地频差估计、生命周期复验及连续 DCO 更新，不授予锁相 | 4 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_match.inc | codec/绑定/STOP/直接入口/精确历史/频差区间/真实 Domain host、目标资源、匹配源码四板 P3 与本地 DCO 专项 | 2026-09-17 | pending |
