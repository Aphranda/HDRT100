...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱与运行代际；Core1 编码/直接匹配、TDMA 双缓冲及 RX IRQ 交接；NO1 有限共钟半开求交、编码后提交与原生重放；typed 共钟差分与旧阶梯界分离，有界窄基线与档位复评；STOP 配置、绑定锁存及显式 Flash 保存；可选本地相位精确平移，以完整发布回执授权同 rate epoch 的累计平移归一化，频率更新/未知模型/STOP 取消；输出独立有符号 delay 配置与持久化；精确整数反解与共同未来网格准备，SYNC_IO 统一执行特等席 PIO 边沿，不授予 RUN 或锁相 | 12 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_match.inc | codec/绑定/STOP/直接入口/历史/共钟及 Domain oracle、频率/相位共存与完整回执、原生 schema/配置锁存/Flash 兼容恢复、目标资源、同源码四板 P3 与专项 | 2026-09-17 | pending |
