...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱与运行代际；Core1 编码/直接匹配，TDMA 双缓冲及 RX IRQ 交接；NO1 有限共钟桥接半开求交、编码后提交、矛盾退休与连续原生重放；typed 连续事件共钟差分保留双层整数取整和绝对准入，旧微秒阶梯余量独立保留；有界复评、复验后连续更新 DCO，不授予锁相 | 8 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_match.inc | codec/绑定/STOP/直接入口/精确历史/共钟求交及真实 Domain oracle/连续事件与旧阶梯差分/原生双 schema/有界复评、目标资源、同源码四板 P3 与专项 | 2026-09-17 | pending |
