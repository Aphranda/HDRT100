...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱、output-ns 区间与非复用运行代际；Core1 编码、TDMA owner 双缓冲提交与 RX IRQ 有界同步交接，普通工位不可覆盖，不授予 DCO/锁相 | 2 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_rx.inc | codec/运行绑定/STOP/普通隔离/直接入口 host、目标资源、匹配源码四板 P3 与 typed 接收专项 | 2026-09-17 | pending |
