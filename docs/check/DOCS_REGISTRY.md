...
| VDC-PRIORITY-01 | vdc | 固定 typed 同步邮箱与运行代际；Core1 编码/直接匹配，TDMA 双缓冲及 RX IRQ 交接；同模型相关差分保留量化界并求交，宽区间跨死区时按有限档位延长基线，计算前消费档位，复验后连续更新 DCO，不授予锁相 | 6 | docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01 | vdc_priority_match.inc | codec/绑定/STOP/直接入口/精确历史/相关差分/有界复评与真实 Domain oracle、目标资源、同源码四板 P3 与原生 DCO 专项 | 2026-09-17 | pending |
