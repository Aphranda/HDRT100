...
Status: Frozen
Domain: TDMA
Canonical: `docs/legacy/tdma/LEGACY_TDMA_DOMAIN_TODO.md`
...
> 历史快照，保留原 task ID、状态及验收描述；当前状态以 `docs/tdma/TDMA_DOMAIN_TODO.md` 为准。
...
以下事项归 TDMA 后续优化，均不作为 DPLL 推进或 `VDC-TIME-003/004` 的前置。
直接复用已验收飞行数据通路；DPLL 在预热后消费有效时间样本，偶发坏帧跳过，
不因单帧直接重锁。只有实测定位到具体 TDMA 缺陷破坏有效时间输入时，才将对应
修复调回当前切片，不扩大为全部启动证明。决策及既有证据索引见
`docs/vdc/VDC_TASK_PROGRESS.md` 的 `VDC-PROGRESS-20260915-049`，当前执行方案见
`docs/vdc/VDC_STABLE_INPUT_PLAN.md`；历史失败、已有代码及证据保留。

| ID | 任务 | 状态 | 完成或退出门禁 |
|---|---|---|---|
| `TDMA-REFINE-001` | 精细核对启动唯一发车、首描述符与首档的对应关系。 | PENDING | 复用实际 builder 和原件完善启动诊断；首失败保持原失败，不以较晚成功帧替换。不得要求 DPLL 使用首帧。 |
| `TDMA-REFINE-002` | 分解实际分频下 capture/sequence/control 采样偏移、完整性与 stall 边界。 | PENDING | 区分数据采样错误与有界采样时差，用实测精度预算确定优化收益；不要求各路径同一系统周期采样。 |
| `TDMA-REFINE-003` | 完善首原始窗口不跨 CS 拼接的物理身份诊断。 | PENDING | 复用首 RX 档案与边界原件完成旁路诊断，不阻止后续有效帧进入 DPLL。 |
