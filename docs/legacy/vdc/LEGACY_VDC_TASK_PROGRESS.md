...
Status: Frozen
Domain: VDC
Canonical: `docs/legacy/vdc/LEGACY_VDC_TASK_PROGRESS.md`
...
| 接口 | 规则 |
|---|---|
| 当前 checkpoint | 只记录当前正在推进的 TODO Task ID、证据状态和下一 gate。 |
| 任务记录 | 每条新增记录必须包含唯一 `Progress ID`、对应 TODO Task ID、日期、变更/提交、验证、结果、证据位置和下一步。 |
| 历史记录 | 2026-09-06 之前的记录保留为 legacy evidence；不回写当前 TODO 状态，不作为架构事实源。 |
| 证据边界 | replay、host 测试、HIL、OTA、构建和失败原始输出只在本文件或 `out/` 中引用；不得在 Architecture 中复制。 |
...
- Progress ID：`VDC-PROGRESS-YYYYMMDD-NNN`
- TODO task ID：`VDC-...`
- 状态：
- 日期：
- 任务目标：
...
VDC Domain 当前 checkpoint 是按 `VDC_DOMAIN_TODO.md` 的 HAOFV 执行顺序，先闭环 TDMA/Calibration/evidence，再推进粗锁、目标锁、稳定 snapshot 和 RUN gate：
...
历史记录保留早期文档主域建设和诊断字段收敛过程；当前任务不得把诊断 evidence 冒充正式目标锁定。

## 任务记录

### VDC-TASK-20260906-002 - TDMA 确定性通信同步方法与 HAOFV TODO 重排

- Progress ID：`VDC-PROGRESS-20260906-002`
- TODO task ID：`VDC-TDMA-001`、`VDC-CAL-001`、`VDC-EVID-001`、`VDC-SERVO-001`、`VDC-SERVO-002`、`VDC-LOCK-001`
- 状态：DONE
- 日期：2026-09-06
- 任务目标：
  - 将 EtherCAT Distributed Clocks、IEEE 1588 hardware timestamp/peer delay、White Rabbit 频率/相位分层、TSN/gPTP source health 和 time-triggered communication 的可借鉴机制收敛到 TDMA-VDC 架构。
  - 按 HAOFV owner/dependency 重新排列 VDC TODO，禁止在 TDMA、Calibration 和 timestamp evidence 未闭环前直接调 DPLL 或发布正式锁。
- 完成内容：
  - `VDC_DOMAIN_ARCHITECTURE.md` 增加 TDMA 固定 process image/trailer、T1/T2/T3/T4、path-delay matrix、跨周期频率估计、FLL-assisted acquisition、Type-II PI tracking、lock promotion 和故障恢复边界。
  - `VDC_DOMAIN_TODO.md` 增加唯一 HAOFV 执行顺序：TDMA → Calibration → evidence → FLL 粗锁 → PI 目标锁 → VdcVector → HOLDOVER/RELOCK → RUN gate → HIL；每项绑定 owner、依赖和退出门禁。
  - 明确 `VDC_DOMAIN_LOCK_LOCKED` 只是环路状态，`FORMAL_LOCKED` 必须由 fine quality、formal timestamp gate、freshness、active calibration 和非 provisional path 共同提升。
- 验证结果：
  - `python tools/docs_check/docs_check.py --strict-names` 通过。
  - `python tools/doc_regression_check.py` 通过；保留既有旧 contract ID 格式 warning。
  - `python -m pytest tests/python/test_doc_regression.py tests/python/test_docs_check.py -p no:cacheprovider`：18 passed。
  - Git Bash 执行 `.githooks/pre-commit` 通过，P3 hardware gate 因本次无 firmware-scope changes 跳过。
- 还需完成：
  - 在代码中实现 `coarse_locked/formal_locked` promotion gate 和正式 timestamp resolution 分层。
  - 用当前 build 做 TDMA feedback、粗锁、目标锁和 HOLDOVER/RELOCK 的硬件闭环验收。
- 关联文件：
  - `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md`
  - `docs/vdc/VDC_DOMAIN_TODO.md`
  - `docs/vdc/VDC_TASK_PROGRESS.md`
- 下一步：先完成 `VDC-TDMA-001`、`VDC-CAL-001`、`VDC-EVID-001`，再进入 `VDC-SERVO-001/002` 参数验证。
