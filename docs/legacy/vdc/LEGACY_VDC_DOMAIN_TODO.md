...
Status: Frozen
Domain: VDC
Canonical: `docs/legacy/vdc/LEGACY_VDC_DOMAIN_TODO.md`
...
| 接口 | 唯一职责 |
|---|---|
| `VDC_DOMAIN_ARCHITECTURE.md` | 稳定语义、owner、不变量、数据模型、跨域契约和失败恢复。 |
| 本文 | 稳定 Task ID、当前状态、进入/退出门禁、依赖顺序和未完成工作。 |
| `VDC_TASK_PROGRESS.md` | 按时间追加实施 checkpoint、验证命令、原始证据、失败/回退和下一 gate。 |

任务状态只使用 `DONE`、`IN PROGRESS`、`PENDING`、`BLOCKED`。单次 build、板端计数和
诊断 replay 结果只能进入 Task Progress，不得改变本文任务状态或架构契约。当前文档中的
旧 `P0–P8`、`R0–R6` 和日期段落保留为追溯映射；新的执行顺序以“HAOFV 执行顺序（唯一入口）”表为准。

阶段性事实快照（非架构事实源）：四节点 TDMA Foundation、NO5 只读监视器、PIO/DMA completion-latch 和 VDC/DPLL vector 发布路径已有 bring-up 证据；active Calibration `PATH_DELAY`、正式 timestamp eligibility 和目标锁定仍以 `VDC_TASK_PROGRESS.md` 的最新证据为准，不能由旧快照标记为 `LOCKED` 或 `HEALTHY`。
...
本表是当前 TODO 的唯一执行顺序。后面的历史 P0–P8、R0–R6 和阶段性说明只用于追溯，
不得跳过前置 owner 的退出门禁直接调 servo 或发布目标锁。状态只使用 `DONE`、`IN PROGRESS`、
`PENDING`、`BLOCKED`；证据记录进入 `VDC_TASK_PROGRESS.md`，稳定语义进入架构文档。

| 顺序 | Task ID | HAOFV owner | 任务 | 状态 | 进入条件 | 完成/退出门禁 |
|---:|---|---|---|---|---|---|
| 1 | `VDC-TDMA-001` | TDMA Foundation | 固定 process image、UP/DOWN 同周期常驻、ring sequence/CRC、PIO/DMA timestamp latch。 | IN PROGRESS | TDMA profile 已 activation。 | `up_running/down_running`、sequence、completion、资源和错误证据连续有效；VDC 不拥有 transport。 |
| 2 | `VDC-CAL-001` | Calibration + VdcSyncAO | 导入 active path-delay、bias、generation/freshness 和完整 observation matrix。 | IN PROGRESS | TDMA schedule/profile CRC 一致。 | matrix 完整、table CRC/generation/freshness 通过；缺项 fail-closed，不沿环推断。 |
| 3 | `VDC-EVID-001` | VdcSyncAO + Timestamp service | 将硬件 latch fact 展开为同圈、同序、同 CRC 的 DPLL evidence。 | IN PROGRESS | `VDC-CAL-001` 通过。 | 正式证据满足 `HARDWARE_TICK`、正式 resolution limit、`DPLL_ELIGIBLE`、非 diagnostic-only、窗口和 payload gate。 |
| 4 | `VDC-SERVO-001` | SyncDpllFB | FLL-assisted acquisition：多周期 phase slope、受限初始 step/feed-forward、频率限幅。 | PENDING | `VDC-EVID-001` 有连续合格样本。 | 进入 `FREQ_LOCK`，频率误差方向稳定；只发布 `COARSE_LOCKED`/tracking candidate。 |
| 5 | `VDC-SERVO-002` | SyncDpllFB | 二阶 Type-II PI 精确跟踪：`kp_q16/ki_q16`、anti-windup、phase/rate slew 和 residual 统计。 | PENDING | `VDC-SERVO-001` 退出。 | `PHASE_LOCK` 稳定，DCO snapshot 可由 core1 完整消费；不把 correction 后 residual 当质量证据。 |
| 6 | `VDC-LOCK-001` | VdcQualityGateFB + VdcSyncAO | 粗锁/目标锁 promotion gate，分离 `COARSE_LOCKED`、`TRACKING_CANDIDATE`、`FORMAL_LOCKED`。 | PENDING | `VDC-SERVO-002` 运行。 | formal lock 同时满足 fine tier、连续窗口、RMS/peak/jitter、freshness、active calibration、正式 timestamp gate 和非 provisional path。 |
| 7 | `VDC-SNAPSHOT-001` | VdcVector + core1 realtime | 发布带 guard/sequence 的 clock/DCO/quality snapshot，core1 只消费稳定版本。 | PENDING | `VDC-LOCK-001` 有 promotion 结果。 | 半更新、stale、generation mismatch、late snapshot 均 fail-closed；不写回 VDC owner 字段。 |
| 8 | `VDC-HOLD-001` | VdcSyncAO + VdcQualityGateFB | HOLDOVER aging、dispersion/drift bound、RELOCKING 和 FAULT 路径。 | PENDING | snapshot 和 quality 已稳定。 | 无新样本时不伪造锁；超预算禁止 RUN；恢复必须重新经过 acquisition/target promotion。 |
| 9 | `VDC-RUN-001` | Trigger/RefMem/SystemManager | 将 formal VDC gate 接入 RefMem mirror、T2/READY 和 FIRE_LOAD/RUN。 | PENDING | `VDC-LOCK-001`、`VDC-SNAPSHOT-001`、`VDC-HOLD-001` 通过。 | coarse/provisional/holdover 超预算/unlocked 均拒绝正式 FIRE_LOAD；RefMem 只镜像。 |
| 10 | `VDC-VERIFY-001` | 主控验收 + NO5 只读观测 | replay、故障注入、两板/四板 HIL、长稳和报告归档。 | IN PROGRESS | 前九项按阶段开放。 | 诊断 replay 与正式硬件验收分开；报告同时给出粗锁、目标锁、gate、原始证据和失败原因。 |

执行规则：

- `VDC-TDMA-001`、`VDC-CAL-001`、`VDC-EVID-001` 未通过时，不得以调大 `kp_q16/ki_q16` 或放宽 lock threshold 掩盖输入问题。
- `VDC-SERVO-001/002` 只由 `SyncDpllFB` 写 offset/rate/lock/DCO；SCPI、NO5、RefMem 和 host 工具没有写权限。
- `VDC-LOCK-001` 未通过时，state=`LOCKED` 只能解释为 profile/bring-up 状态，不能解释为 `HEALTHY` 或正式 RUN。
- 每个 Task ID 的一次执行结果、build/HIL、失败、回退和证据路径追加到 `VDC_TASK_PROGRESS.md`。
...
| P0.5 | 实现 `SyncDpllFB` 最小环路滤波器，使用 accepted sample 计算 phase error / frequency error，经 PI 或等价 servo、限幅和 reset policy 调节 VDC clock model 的 offset/rate，并发布 DCO snapshot。 | 按 `VDC-SERVO-001` 先做 FLL-assisted acquisition，再按 `VDC-SERVO-002` 做二阶 Type-II PI 跟踪；`INITIAL_SYNC/FREQ_LOCK/PHASE_LOCK/LOCKED` 只描述环路状态，粗锁/目标锁由 `VDC-LOCK-001` promotion gate 分离。`phase_diagnostic_threshold_ns` 只写 quality flag，不再使用失效的 `SERVO_OUTLIER`/`outlier_threshold_ns` 叙述；formal lock 必须由连续 fine sample、RMS/peak/jitter、freshness、active calibration 和正式 timestamp gate 共同证明。 |
