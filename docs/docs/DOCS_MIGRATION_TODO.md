...
- [x] P1-09 冻结系统/项目命名层级：系统/项目名为 `Distributed Hard Real-Time Trigger System`，
  `DHRT100` 保留为当前设备型号，`RP2350 / RP2350B` 保留为当前硬件平台，
  `RP2350_TRIG` 保留为历史工程和构建产物名。
- [x] P1-10 建立未来应用路线图入口：新增 `ARCH_FUTURE_APPLICATION_PLAN.md`，
  记录当前产品完成后的应用场景、跨平台移植、版本分层和开源生态方向，不进入近期实现待办。
...
| 2026-08-13-PROJECT-NAME-FREEZE | 已完成 | `DHRT100 / RP2350_TRIG` 作为项目/系统名 | `Distributed Hard Real-Time Trigger System` | 已更新 README、HAOFV、产品架构、RTOS、RefMem、SCPI 规划和文档治理入口；`DHRT100` 型号、`RP2350` 硬件平台、`RP2350_TRIG` 构建产物名暂不迁移。 |
...
| 2026-07-07-NAMING-BATCH-2 | 已完成 | `PORTABLE_OTA_ARCHITECTURE.md` | `docs/legacy/ota/OTA_PORTABLE_ARCHITECTURE.md` | 已更新 README、docs、tools 内引用；归入 OTA 架构文档。 |
...
| 2026-07-07-NAMING-BATCH-3 | 已完成 | `OTA方案.md` | `docs/legacy/ota/OTA_SYSTEM_DESIGN.md` | 已更新 README、docs、tools 内引用；保留 OTA 主方案 canonical 入口语义。 |
