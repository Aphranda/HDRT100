...
| `VDC-PRIORITY-01` | typed 同步邮箱携带完整事件/运行代际及 output-ns 区间，Core1 编码/直接匹配、TDMA 双缓冲及 RX IRQ 交接；NO1 有限共钟半开求交、逐帧 latch、编码后提交与原生重放；typed 共钟差分与旧阶梯界分离，有界窄基线和档位复评；STOP 配置、绑定锁存与 Core0 显式 Flash 保存；可选本地相位精确平移以完整发布回执授权累计平移归一化，频率变化/未知模型/STOP 取消；独立输出 delay 可配置并持久化，RUN 物理边沿、单圈期限、ACK 与锁相另行验收 | `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01` | pending |
