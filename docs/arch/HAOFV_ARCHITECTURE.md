...
| `VDC-PRIORITY-01` | 独立 typed 同步邮箱携带完整事件/运行代际和 output-ns 区间；Core1 编码、直接匹配，TDMA 双缓冲及 RX IRQ 交接；NO1 共钟桥有限半开求交，保留逐帧 latch，编码后提交缓存并连续原生留证，矛盾退休；typed 连续事件共钟差分保留双层整数取整与绝对准入，旧微秒阶梯余量独立保留；有界复评，经生命周期复验后连续更新 DCO；单圈期限、ACK、相位与锁相分别验收 | `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01` | pending |
