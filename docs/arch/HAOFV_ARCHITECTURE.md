...
汇总显式扩窗通过 STOP 下的双参数 ARM 选择独立版本，旧单参数版本保持原间隔；
跨度受整倍数、静态上限和当前 tick rate 可表示范围约束，经 request/ACK 一致提交。
扩窗不增加记录池，也不放宽服务缺口判据；计数/间隔饱和及覆盖不完整仍显式保留。
origin 汇总计数基线绑定当前 sync generation，不混用启动前旧代计数；当前代
首次拒绝、同代回退、读取失败及运行中异代均按真实 owner 观察保留。

可选诊断 guard 在 Core1 按固定检查点联合判定参考与输出引擎健康。首次运行绑定
已准备的输出请求，检查点只读核对身份、状态、新鲜度与末序号推进；参考正常
不能掩盖输出退休，读取失败不复用旧通过。观测不服务硬件，不是 GPIO 测量；
最终通过后至统一停止的尾段仍须独立审计。Core0 仅在原环路配置
与 capture/session/generation 一致时，经 TDMA control guard 请求失败本板停止；
目标通过前封存末段并复验最后的时钟/计数器/覆盖，以独立目标完成原因冻结原生记录；
封存不停止输出或环路。完成等待后由诊断编排统一 STOP，避免先通过者截断其余节点。
目标后的参考不再记录，输出退休尾段仍单独审计，不能外推未观测区间的参考或 GPIO 质量。
...
整段 TDMA 服务因本相位余量不足而跳过时，可按独立候选预算优先准入输出规划/
补给，余量更小时才准入已有缓存交接；两者互斥且不越过原相位关闭点。模型改变
后的失效后缀必须重建，不重放旧缓存；完整服务的跳过和失败仍留账，各路径分别
测量并检查预算。该补给机会不等同 WCET 或连续性证明，不新增调度相位或 Core0
实时依赖。细节见 `VDC-PRIORITY-01` 域契约与 `vdc_run_output.h` 的版本化诊断。
