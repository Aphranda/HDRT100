...
| `VDC-PRIORITY-01` | 独立 typed 同步邮箱携带完整事件/运行代际和 output-ns 区间；Core1 编码、直接匹配，TDMA 双缓冲及 RX IRQ 交接；本地同模型共起点差分保留量化界并与绝对端点求交，互斥频差估计经生命周期复验后连续更新 DCO；单圈期限、ACK、相位与锁相分别验收 | `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01` | pending |
