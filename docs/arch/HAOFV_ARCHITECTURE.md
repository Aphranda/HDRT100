...
| `VDC-PRIORITY-01` | 独立 typed 同步邮箱携带完整事件/运行代际和 output-ns 区间；Core1 编码、直接匹配，TDMA 双缓冲及 RX IRQ 交接；同模型相关差分保留量化界并求交，宽跨死区基线按有限档位延长、计算前消费档位，经生命周期复验后连续更新 DCO；单圈期限、ACK、相位与锁相分别验收 | `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01` | pending |
