...
| `VDC-PRIORITY-01` | 独立 typed 同步邮箱携带完整事件/运行代际和 output-ns 区间；Core1 编码，TDMA owner 双缓冲提交，RX IRQ 在普通队列前交接；Core1 直接索引源事件，显式互斥本地频差估计经生命周期复验后连续更新 DCO；单圈期限、ACK、相位与锁相分别验收 | `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01` | pending |
