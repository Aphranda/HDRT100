...
| `VDC-PRIORITY-01` | 独立 typed 同步邮箱携带完整事件/运行代际和 output-ns 区间；Core1 编码，TDMA owner 双缓冲提交，RX IRQ 在普通队列前交接；Core1 直接索引源事件、复验生命周期并计算暂定定向 delay 残差；DCO、单圈期限及锁相分别验收 | `docs/vdc/VDC_DOMAIN_ARCHITECTURE.md:VDC-PRIORITY-01` | pending |
