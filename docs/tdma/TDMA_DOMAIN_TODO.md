...
流程总时限拒绝继续开放。下一切片先分解 RESET 首拍的 snapshot 初始化、work
...
有界退休方案须保持 Core1 写所有权、Core0 单次快照、reset generation、未执行分项
与空记录输出语义，再验证对完整外层峰值的影响。不得删除探针或混加不同拍的单项
最大值；随后继续 RX/latch 有效工作与布局、缓存/总线/IRQ 干扰归因。
