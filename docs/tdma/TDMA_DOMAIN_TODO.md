...
| TDMA-FLIGHT-002B | B1 自激发车时钟：Core1 退出发车门控 | IN PROGRESS | 屏蔽 core1 service 后环持续运行；`emission_clock_source` 为环边界来源；帧间隔 min/max 落在固定拍数窗口内。follower 续转与单 pending generation 交接见 `TDMA-PROGRESS-20260912-003`。reference header 计算候选与返回 ring 反例已离线验证，见 `TDMA-PROGRESS-20260912-004`；下一步先闭合硬件帧位置/返回映像交接、origin command DMA/sniffer 仲裁和 CONTROL/DATA 边界，之后再集成发车与完整 completion。离线模型及局部实现不代表本项通过。 |
