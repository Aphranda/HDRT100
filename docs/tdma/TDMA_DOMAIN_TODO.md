...
| TDMA-FLIGHT-002B | B1 自激发车时钟：Core1 退出发车门控 | IN PROGRESS | 屏蔽 core1 service 后环持续运行；`emission_clock_source` 为环边界来源；帧间隔 min/max 落在固定拍数窗口内。follower 续转见 `TDMA-PROGRESS-20260912-003`；reference 头部审计及完整 origin 图、返回池/影子池交接、窄 RX/CRC 和固定准备窗口候选见 `TDMA-PROGRESS-20260912-004/005`。下一步集成完整 C builder、独立 executor/sniffer 准入、异步 completion 与 owner 生命周期；校验分支耗时变化和准备迟到必须受 Calibration 预算与有界故障策略约束。离线模型不能替代当前源码 P3、多板原始波形、完整 WCET 或真实 Core1 service blackout。 |
