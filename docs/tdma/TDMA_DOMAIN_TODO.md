...
当前 RX 取帧内部归因见 `TDMA-PROGRESS-20260912-025`：版本化探针区分 DMA 观察、
定位、header 校验与 ring copy。实板证据将逐字归一化复制列为下一优先项；真实镜像
显示其位反转仍逐字调用外部函数，候选定位的除法开销优先级较低。下一切片先验证
内联同语义 bit reverse，在相同计时版本下重新构建、验收和测量，不凭指令形态承诺收益。
本轮探针增加链接 RAM，正式 RAM/WCET 及 SCK 候选覆盖、ARM/STOP 恢复缺陷仍开放；
本次严格矩阵、生命周期与普通/自主有限原件通过不等于列车阶段通过。
随后继续调查 owner/adapter 交接、代码布局、共享访问和 IRQ 干扰，以及无更新时的
