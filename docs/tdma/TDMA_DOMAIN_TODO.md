...
| TDMA-FLIGHT-002B | B1 自激发车时钟：Core1 退出发车门控 | IN PROGRESS | 屏蔽 core1 service 后环持续运行；`emission_clock_source` 为环边界来源；帧间隔 min/max 落在固定拍数窗口内。follower 续转见 `TDMA-PROGRESS-20260912-003`；origin 图与交接/固定窗口候选见 `TDMA-PROGRESS-20260912-004/005`。STOP 依赖链、拒绝传播与停止态调度竞争的前置修复及硬件门禁见 `TDMA-PROGRESS-20260912-006`。先完成该生命周期闭环，再集成完整 C builder、executor/sniffer 准入、异步 completion 与 owner 生命周期；准备窗口及迟到故障策略须进入 Calibration 预算。离线模型不能替代当前源码 P3、多板原始波形、完整 WCET 或真实 Core1 service blackout。 |
