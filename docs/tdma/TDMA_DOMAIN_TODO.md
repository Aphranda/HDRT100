...
下一切片优先实现板端时钟驱动的固定容量记录，保留健康字段、序列、配置 identity、
采样/覆盖/取消证据，完成后统一导出。先核查现有 SD RUN 保存的资源等待，再由 Core0
Storage owner 批量落盘；不得让 Core1 等待 SD/Flash 或用异步存储掩盖采样丢失。
原五查询仅作迁移对照，板端记录具备等价证据后再切换验收入口。
后续性能切片继续拆分
