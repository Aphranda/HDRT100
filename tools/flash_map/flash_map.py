...
def validate_map(source: dict[str, Any]) -> dict[str, Any]:
    ...
    if source["deployment_state"] not in {"target_not_deployed", "factory_validation", "deployed"}:
    ...
    for required in ("BOOTLOADER", "BOOT_CONTROL", "APP_A", "APP_B", "RECOVERY", "SCRATCH", "FUTURE_POOL"):
...
def render_header(data: dict[str, Any], source_path: Path) -> str:
    ...
    lines = [
        ...
        f"#define FLASH_GEOMETRY_XIP_BASE {hex_u(geometry['xip_base'])}",
        "",
...
def render_cmake(data: dict[str, Any], source_path: Path) -> str:
    geometry = data["geometry"]
    lines = [
        "# Generated by tools/flash_map/flash_map.py; do not edit.",
        f"# Source: {source_path.as_posix()} ({data['deployment_state']}).",
        f"set(FLASH_MAP_VERSION {data['map_version']})",
        f"set(FLASH_GEOMETRY_TOTAL_SIZE_BYTES {geometry['total_size']})",
        f"set(FLASH_GEOMETRY_XIP_BASE 0x{geometry['xip_base']:08X})",
    ]
...
def render_ld(data: dict[str, Any], source_path: Path) -> str:
    geometry = data["geometry"]
    lines = [
        "/* Generated by tools/flash_map/flash_map.py; do not edit. */",
        f"/* Source: {source_path.as_posix()} ({data['deployment_state']}). */",
        f"FLASH_GEOMETRY_XIP_BASE = 0x{geometry['xip_base']:08X};",
        f"FLASH_GEOMETRY_TOTAL_SIZE = 0x{geometry['total_size']:08X};",
    ]
    for partition in data["partitions"]:
        prefix = f"FLASH_MAP_{partition['id']}"
...
def main() -> int:
    ...
    try:
        ...
        outputs = {
            ...
            args.cmake: render_cmake(data, relative_source),
            args.ld: render_ld(data, relative_source),
        }
