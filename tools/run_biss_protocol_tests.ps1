...
$include = Join-Path $repo "components\sync_trigger\inc"
$testSource = Join-Path $repo "tests\unit\test_biss_protocol.c"
$protocolSource = Join-Path $repo "components\sync_trigger\src\biss_protocol.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_biss_protocol.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$include" $testSource $protocolSource -o $exe
    ...
    Write-Host "biss_protocol host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $protocolSource)) {
...
Write-Host "biss_protocol tests compiled with ARM GCC; host execution skipped because no host C compiler was found"
