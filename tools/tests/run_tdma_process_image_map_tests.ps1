...
$testSource = Join-Path $repo "tests\unit\test_tdma_process_image_map.c"
$mapSource = Join-Path $repo "components\tdma\src\tdma_process_image_map.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_tdma_process_image_map.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$include" $testSource $mapSource -o $exe
    ...
    Write-Host "tdma_process_image_map host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $mapSource)) {
...
Write-Host "tdma_process_image_map tests compiled with ARM GCC; host execution skipped"
