...
$testSource = Join-Path $repo "tests\unit\test_calibration_clk_marker.c"
$sources = @(
    (Join-Path $repo "components\calibration_manager\src\calibration_clk_marker.c"),
    (Join-Path $repo "components\calibration_manager\src\calibration_clk_coded.c")
)
...
if ($hostCc) {
    $exe = Join-Path $build "test_calibration_clk_marker.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$include" `
        $testSource $sources -o $exe
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $exe
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host "calibration_clk_marker host unit tests passed"
    exit 0
}
...
foreach ($file in @($testSource) + $sources) {
    $object = Join-Path $build ((Split-Path -Leaf $file) + ".o")
    & $ArmGcc -std=c11 -Wall -Wextra -Werror "-I$include" -c $file -o $object
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}

Write-Host "calibration_clk_marker tests compiled with ARM GCC; host execution skipped"
