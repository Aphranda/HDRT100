param(
    [string]$BuildDir = "out/pytest/build-refmem-sync-frame-tests",
    [string]$ArmGcc = ""
)
...
$testSource = Join-Path $repo "tests\unit\test_refmem_sync_frame.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_refmem_sync_frame.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$refmemInclude" "-I$tdmaInclude" "-I$otaInclude" $testSource $syncFrameSource -o $exe
    ...
    Write-Host "refmem_sync_frame host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $syncFrameSource)) {
...
Write-Host "refmem_sync_frame tests compiled with ARM GCC; host execution skipped because no host C compiler was found"
