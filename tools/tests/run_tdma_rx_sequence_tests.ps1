param(
    [string]$BuildDir = "out/pytest/build-tdma-rx-sequence-tests",
    [string]$HostGccDir = "",
    [string]$ArmGcc = ""
)
...
$testSource = Join-Path $repo "tests\unit\test_tdma_rx_sequence.c"
$sequenceSource = Join-Path $repo "components\tdma\src\tdma_rx_sequence.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_tdma_rx_sequence.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$include" `
        $testSource $sequenceSource -o $exe
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $exe
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host "tdma_rx_sequence host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $sequenceSource)) {
...
Write-Host "tdma_rx_sequence tests compiled with ARM GCC; host execution skipped"
