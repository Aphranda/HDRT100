param(
    [string]$BuildDir = "out/pytest/build-refmem-pio-spi-adapter-tests",
    [string]$ArmGcc = ""
)
...
$testSource = Join-Path $repo "tests\unit\test_refmem_pio_spi_adapter.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_refmem_pio_spi_adapter.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$refmemInclude" "-I$tdmaInclude" "-I$otaInclude" $testSource $adapterSource $syncFrameSource -o $exe
    ...
    Write-Host "refmem_pio_spi_adapter host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $adapterSource, $syncFrameSource)) {
...
Write-Host "refmem_pio_spi_adapter tests compiled with ARM GCC; host execution skipped because no host C compiler was found"
