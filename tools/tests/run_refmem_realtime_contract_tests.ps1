param(
    [string]$BuildDir = "out/pytest/build-refmem-realtime-contract-tests",
    [string]$ArmGcc = ""
)
...
$testSource = Join-Path $repo "tests\unit\test_refmem_realtime_contract.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_refmem_realtime_contract.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$refmemInclude" "-I$tdmaInclude" "-I$otaInclude" $testSource $contractSource $claimSource -o $exe
    ...
    Write-Host "refmem_realtime_contract host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $contractSource, $claimSource)) {
...
Write-Host "refmem_realtime_contract tests compiled with ARM GCC; host execution skipped because no host C compiler was found"
