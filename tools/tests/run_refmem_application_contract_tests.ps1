param(
    [string]$BuildDir = "out/pytest/build-refmem-application-contract-tests",
    [string]$ArmGcc = ""
)
...
if ($hostCc) {
    ...
    Write-Host "refmem_application_contract host unit tests passed"
    exit 0
}
...
Write-Host "refmem_application_contract tests compiled with ARM GCC; host execution skipped because no host C compiler was found"
