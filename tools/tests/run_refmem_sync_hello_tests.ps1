param(
    [string]$BuildDir = "out/pytest/build-refmem-sync-hello-tests",
    [string]$ArmGcc = ""
)
...
$testSource = Join-Path $repo "tests\unit\test_refmem_sync_hello.c"
$helloSource = Join-Path $repo "components\distributed_refmem\src\refmem_sync_hello.c"
$syncFrameSource = Join-Path $repo "components\distributed_refmem\src\refmem_sync_frame.c"
$adapterSource = Join-Path $repo "components\distributed_refmem\src\refmem_pio_spi_adapter.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_refmem_sync_hello.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$refmemInclude" "-I$tdmaInclude" "-I$otaInclude" $testSource $helloSource $syncFrameSource $adapterSource -o $exe
    ...
    Write-Host "refmem_sync_hello host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $helloSource, $syncFrameSource, $adapterSource)) {
...
Write-Host "refmem_sync_hello tests compiled with ARM GCC; host execution skipped because no host C compiler was found"
