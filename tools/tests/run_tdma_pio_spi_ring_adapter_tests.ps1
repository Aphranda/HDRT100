...
if ($hostCc) {
    ...
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$include" `
        $testSource $adapterSource $runtimeSource $transportSource $profileSource `
        -o $exe
...
foreach ($source in @($testSource, $adapterSource, $runtimeSource, $transportSource, $profileSource)) {
