param(
    [string]$BuildDir = "out/pytest/build-tdma-transport-frame-tests",
    [string]$HostGccDir = "",
    [string]$ArmGcc = ""
)
...
$testSource = Join-Path $repo "tests\unit\test_tdma_transport_frame.c"
$frameSource = Join-Path $repo "components\tdma\src\tdma_transport_frame.c"
...
if ($hostCc) {
    $exe = Join-Path $build "test_tdma_transport_frame.exe"
    & $hostCc -std=c11 -Wall -Wextra -Werror "-I$include" $testSource $frameSource -o $exe
    ...
    Write-Host "tdma_transport_frame host unit tests passed"
    exit 0
}
...
foreach ($source in @($testSource, $frameSource)) {
...
Write-Host "tdma_transport_frame tests compiled with ARM GCC; host execution skipped"
