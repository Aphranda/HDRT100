...
def configure_commands(args: argparse.Namespace) -> list[str]:
    commands = [
        ...
        biss_command(f"FRAMe:BITS {args.frame_bits}"),
        biss_command(f"POSition:OFFSet {args.position_offset}"),
        biss_command(f"POSition:BITS {args.position_bits}"),
        biss_command(f"POSition:MODulo {args.position_modulo}"),
...
def configure_steps(args: argparse.Namespace) -> list[tuple[str, str | None, object | None]]:
    steps: list[tuple[str, str | None, object | None]] = [
        ...
        (biss_command(f"FRAMe:BITS {args.frame_bits}"),
         biss_command("FRAMe:BITS?"),
         uint_response_matches(args.frame_bits)),
        (biss_command(f"POSition:OFFSet {args.position_offset}"),
         biss_command("POSition:OFFSet?"),
         uint_response_matches(args.position_offset)),
        (biss_command(f"POSition:BITS {args.position_bits}"),
         biss_command("POSition:BITS?"),
         uint_response_matches(args.position_bits)),
        (biss_command(f"POSition:MODulo {args.position_modulo}"),
         biss_command("POSition:MODulo?"),
         uint_response_matches(args.position_modulo)),
