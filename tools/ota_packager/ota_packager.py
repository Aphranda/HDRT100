...
def build_package(
    ...
    *,
    ...
    if len(signature) > 64:
        raise ValueError("signature is limited to 64 bytes in the fixed manifest extension")
    if signature or security_counter or key_id:
        ...
        put_u32(header, MANIFEST_EXTENSION_OFFSET + 8, 1 if signature else 0)
        put_u32(header, MANIFEST_EXTENSION_OFFSET + 12, security_counter)
        put_u32(header, MANIFEST_EXTENSION_OFFSET + 16, key_id)
        put_u32(header, MANIFEST_EXTENSION_OFFSET + 20, len(signature))
